...
\begin{document}
\begin{center}
{\LARGE \textbf{Relational Actualism IV:\\[0.3em] Complexity, Life, and Recursive Closure in Discrete Causal Networks}}\\[1em]
...
\begin{abstract}
This paper extends Relational Actualism (RA) into the domain of complexity, life, and cognition, using the same discrete causal framework developed in earlier papers. Physical reality is modeled as a finite, growing causal graph governed by a BDG acceptance kernel and local conservation constraints. On this basis, the paper investigates the structural conditions under which organized systems can maintain persistent boundary conditions and exhibit recursive closure.

Rather than treating biological or cognitive organization as arising from specific chemical or informational substrates, RA proposes a substrate-independent set of conditions---including redundant boundary inscription and bounded boundary turnover---under which stable, life-like organization can emerge in causal networks. The paper is written from the same doctrine that governs the whole suite: RA's proof path remains native to DAG growth, the BDG acceptance kernel, the local ledger condition, and finite causal organization, while comparison with causal set theory, systems biology, assembly-theoretic approaches, decoherence-based accounts, and consciousness frameworks is made explicit but kept secondary.

The paper advances a structural programme rather than a complete theory, identifying open targets including theorem-level shield laws across coarse-graining levels, persistence-window calculations, direct native computation for prebiotic chemistries, and the conditions for higher-order cognitive organization.
\end{abstract}
...
\begin{itemize}[nosep]
    ...
    \end{itemize}
\end{itemize}
The paper also inherits the severance / boundary logic of Paper~III as suite-level derived support. Markov-blanket theorems, DFT overlays, KCB, Landauer-style bounds, and quantum-information comparison claims are not part of the active native support surface used positively here.
\end{tcolorbox}
...
%% ═══════════════════════════════════════════════════════════
% RAKB: RA-COMP-002;RA-COMP-003;RA-COMP-008
...
% RAKB: RA-COMP-DFT-F1-001;RA-OPEN-007
