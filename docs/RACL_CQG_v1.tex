...
The Planck length $l_P$ acts as a strict UV cutoff \emph{prior} to
taking this asymptotic limit: the vertex density $\lambda$ approaches
the Planck density $\lambda_P = l_P^{-4}$, not mathematical infinity,
so the BDG sum $S_{\mathrm{BDG}}(v) = \sum_k c_k N_k(v)$ remains
finite at all physical densities.  The window $\tau_{\mathrm{BDG}}(\lambda)$
approaches $l_P$ from above rather than reaching zero, ensuring that
the BDG action is local on the scale of a single Planck causal
diamond --- the finest spatial scale with physical meaning in the RA
framework --- without UV divergence.
\end{remark}
...
$H_{\mu\nu} = \alpha\,G_{\mu\nu} + \Lambda\,g_{\mu\nu}$.%
\footnote{Lovelock's theorem applies strictly to the macroscopic
\emph{topological} dimension $D=4$ of the embedding manifold.
It is known that discrete causal graphs can exhibit spectral dimension
fluctuations at microscopic (Planck) scales, with $d_s < 4$ near the
discreteness scale~\citep{Carlip2017,Lauscher2005}.
This does not undermine the argument: the BDG integers $(1,-1,9,-16,8)$
are uniquely determined by 4-dimensional Lorentz invariance
\citep{Benincasa2010}, fixing the macroscopic topological dimension to
exactly $D=4$ independently of microscopic spectral fluctuations.
Lovelock's uniqueness applies in the dense-graph limit
$\lambda\to\lambda_P$, where the continuum embedding is a topological
4-manifold by construction.  Spectral dimension corrections at the
Planck scale appear as higher-order terms in the $\lambda^{-1/2}$
expansion of Theorem~\ref{thm:bd} and are suppressed relative to the
leading $G_{\mu\nu}$ term by $(l_P/L_{\rm macro})^2\sim 10^{-66}$
...
\bibitem[Carlip(2017)]{Carlip2017}
S.~Carlip.
\newblock Dimension and dimensional reduction in quantum gravity.
\newblock \textit{Class.\ Quantum Grav.}, 34:193001, 2017.
\newblock \href{https://doi.org/10.1088/1361-6382/aa8535}{doi:10.1088/1361-6382/aa8535}.

\bibitem[Lauscher \& Reuter(2005)]{Lauscher2005}
O.~Lauscher and M.~Reuter.
\newblock Fractal spacetime structure in asymptotically safe gravity.
\newblock \textit{JHEP}, 10:050, 2005.
\newblock \href{https://doi.org/10.1088/1126-6708/2005/10/050}{doi:10.1088/1126-6708/2005/10/050}.
