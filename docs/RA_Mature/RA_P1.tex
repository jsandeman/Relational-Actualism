...
With amplitude locality proved as a theorem (not assumed as an axiom), the causal invariance of the quantum measure for BDG dynamics is unconditional. The full proof---including the \texttt{List.Perm} induction step establishing permutation-invariance of the multiplicative foldl accumulator---compiles in Lean~4.29 with zero sorry tags (\texttt{bdg\_causal\_invariance} in \texttt{RA\_AmpLocality.lean}~\cite{AmpLocLean}).
...
The BDG coefficients have signs $(+,-,+,-,+)$, forced by the inclusion--exclusion counting of causal intervals. At each depth~$k$, the sign determines whether the corresponding layer contributes isotropically~($c_k > 0$, encouraging many ancestors distributed across spatial directions) or anisotropically~($c_k < 0$, penalising multiple ancestors and selecting a preferred direction):
...
Companion papers derive further consequences: wave function collapse and quantum predictions (Paper~II~\cite{P2}), cosmological parameters and nucleation cosmology (Paper~III~\cite{P3}), and substrate-independent conditions for biological complexity (Paper~IV~\cite{P4}). Paper~0~\cite{P0} provides the narrative overview and complete framework synthesis.
