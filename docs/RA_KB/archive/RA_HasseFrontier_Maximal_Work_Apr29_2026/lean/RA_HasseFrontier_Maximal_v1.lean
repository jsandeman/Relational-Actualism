...
/-- A topological maximum in the reachable upper past is a maximal reachable
vertex.  The proof uses the strict topological increase along nontrivial
reachability: if `f` reached a strictly later `w`, maximality of `f`'s
`topo_order` would be contradicted. -/
