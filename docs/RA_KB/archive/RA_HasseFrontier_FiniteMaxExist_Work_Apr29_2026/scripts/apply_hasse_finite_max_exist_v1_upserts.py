#!/usr/bin/env python3
"""Apply Hasse Frontier FiniteMaxExist v1 upserts to RAKB v0.5 registry."""
...
def main():
    ...
    a=p.parse_args()
    pd=a.packet_root/'registry_upserts_hasse_finite_max_exist_v1'
    upsert_csv(a.registry/'artifacts.csv', pd/'RAKB_hasse_finite_max_exist_v1_artifacts_upsert_Apr29_2026.csv', ['artifact_id'], a.dry_run)
    upsert_csv(a.registry/'claim_artifact_edges.csv', pd/'RAKB_hasse_finite_max_exist_v1_claim_artifact_edges_upsert_Apr29_2026.csv', ['claim_id','artifact_id','relation','source_span'], a.dry_run)
    dest=a.reports/'selector_closure_Apr29_2026'
    copy_report(a.packet_root/'formalization_notes/RA_HasseFrontier_FiniteMaxExist_Programme_Apr29_2026.md', dest, a.dry_run)
    copy_report(a.packet_root/'reports/RA_HasseFrontier_FiniteMaxExist_First_Formalization_Report_Apr29_2026.md', dest, a.dry_run)
