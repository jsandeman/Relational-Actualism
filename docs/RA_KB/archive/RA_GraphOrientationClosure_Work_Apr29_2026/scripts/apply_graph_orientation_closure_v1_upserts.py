...
def upsert(target, patch, key_fn, dry):
    ...
    if not incoming:
        print(f'{patch}: no incoming rows')
        return
    ...
    if not dry:
        if target.exists():
            shutil.copy2(target, target.with_suffix(target.suffix + '.bak_' + datetime.now().strftime('%Y%m%d_%H%M%S')))
...
def main():
    ...
    patch=root/'registry_upserts_graph_orientation_closure_v1'
    upsert(reg/'artifacts.csv', patch/'RAKB_graph_orientation_closure_v1_artifacts_upsert_Apr29_2026.csv', lambda r:r.get('artifact_id',''), args.dry_run)
    upsert(reg/'claim_artifact_edges.csv', patch/'RAKB_graph_orientation_closure_v1_claim_artifact_edges_upsert_Apr29_2026.csv', edge_key, args.dry_run)
    if not args.dry_run:
        out=reports/'selector_closure_Apr29_2026'; out.mkdir(parents=True, exist_ok=True)
        for sub in ['reports','formalization_notes']:
