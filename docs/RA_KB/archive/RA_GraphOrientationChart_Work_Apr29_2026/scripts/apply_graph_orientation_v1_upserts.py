...
def main():
    ...
    patch=root/'registry_upserts_graph_orientation_v1'
    upsert(reg/'artifacts.csv', patch/'RAKB_graph_orientation_v1_artifacts_upsert_Apr29_2026.csv', lambda r:r.get('artifact_id',''), args.dry_run)
    upsert(reg/'claim_artifact_edges.csv', patch/'RAKB_graph_orientation_v1_claim_artifact_edges_upsert_Apr29_2026.csv', edge_key, args.dry_run)
    if not args.dry_run:
        out=reports/'selector_closure_Apr29_2026'; out.mkdir(parents=True, exist_ok=True)
        for p in (root/'reports').glob('*'):
            if p.is_file(): shutil.copy2(p, out/p.name)
        for p in (root/'formalization_notes').glob('*'):
            if p.is_file(): shutil.copy2(p, out/p.name)
