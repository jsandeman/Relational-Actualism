#!/usr/bin/env python3
"""Apply Frontier/Incidence v2 compile-confirmed upserts to RAKB v0.5 registry."""
...
def copy_file(src: Path, dest_dir: Path, dry_run: bool) -> None:
    print(f"copy: {src.name} -> {dest_dir / src.name}")
    if not dry_run:
...
def main() -> None:
    ...
    patch_dir = args.packet_root / "registry_upserts_frontier_v2"
    upsert_csv(
        args.registry / "artifacts.csv",
        patch_dir / "RAKB_frontier_v2_artifacts_upsert_compile_confirmed_Apr29_2026.csv",
        ["artifact_id"],
        args.dry_run,
    )
    upsert_csv(
        args.registry / "claim_artifact_edges.csv",
        patch_dir / "RAKB_frontier_v2_claim_artifact_edges_upsert_compile_confirmed_Apr29_2026.csv",
        ["claim_id", "artifact_id", "relation", "source_span"],
        args.dry_run,
    )

    dest = args.reports / "selector_closure_Apr29_2026"
    copy_file(args.packet_root / "reports" / "RA_FrontierIncidence_CompileConfirmed_Report_Apr29_2026.md", dest, args.dry_run)
    copy_file(args.packet_root / "patches" / "patch_lakefile_add_selector_frontier_roots.diff", dest, args.dry_run)
