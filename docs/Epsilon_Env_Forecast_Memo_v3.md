...
**The sign is unambiguous:** α(void) > α(filament) in both transverse and radial directions, because voids expand faster than filaments (Milne > EdS). The "BAO appears larger in voids" direction is correct when phrased in α.
...
Neither. Both are toys. The true answer requires solving the null geodesic equation in an actual inhomogeneous geometry, which is not a straightforward calculation. Model A tends to understate the signal because it ignores the curvature contribution in genuinely void-dominated portions of the path; Model B tends to overstate it because it treats every mixed path as if both curvatures are fully present.

**For an order-of-magnitude sensitivity target, take the range Δα_⊥^endpoint ∈ [0.14, 0.21] at z = 1.** The radial endpoint Δα_∥ is less ambiguous because it depends only on H at the measurement redshift, not on path-integrated curvature; its endpoint value is 0.258 at z = 1 regardless of toy model.
...
**Revised honest forecast.** Rather than quoting a specific ε_env prediction, I now state:

$$\text{Plausible range: } 0.1 \lesssim \varepsilon_{\text{env}} \lesssim 0.5$$

based on toy-model sensitivity considerations, with no strong prior preference for any specific value in this range. ε_env is the measured parameter, not a forecast quantity.
...
> RA (Relational Actualism) predicts, in a Λ = 0 inhomogeneous cosmology, that cosmic voids approach Milne expansion (q = 0) while matter-dominated regions approach EdS expansion (q = +0.5). Consequently, the observed BAO AP parameters should differ between void-dominated and filament-dominated environments at fixed z, with α(void) > α(filament) in both transverse and radial directions. The endpoint (pure-Milne vs pure-EdS) diagnostic at z = 1 gives Δα_⊥ in the range 0.14–0.21 (depending on curvature treatment of mixed paths) and Δα_∥ ≈ 0.26. The realized DESI signal is expected to be suppressed by an environmental factor ε_env whose rigorous calculation requires inhomogeneous-GR distance machinery not yet developed within RA; toy estimates make values in the range 0.1–0.5 plausible but do not justify a specific prediction. DR1 is suitable for a pilot analysis and mock-calibrated upper limit / marginal detection; DR2 or the void-galaxy anisotropic cross-correlation statistic (Nadathur-style) may be required for decisive tests. The RA contribution to the broader Λ = 0 inhomogeneous-cosmology program is the structural derivation of Λ = 0 from actualization physics (P_act vacuum suppression), which shifts the explanatory question from "why Λ is so small" to "what is the precise observational signature of inhomogeneous backreaction in a universe whose Λ vanishes by construction."
...
- `t1_forecast_deliverables.py` — Python script that generates the tables in §1 and §3
- `Epsilon_Env_Forecast_Memo.md` (v1) — **superseded by this document**; retained for record of the reasoning that led here

The honest computational state: Deliverable 1 (endpoint calculator) is clean and verified. Deliverable 2 (two competing toy models) is a bracketing diagnostic, not a derivation. Deliverable 3 (sensitivity matrix) shows what DR1 can and can't do for a range of ε_env values. The rigorous forecast remains a specific open problem, best addressed through the Wiltshire contact or N-body ray tracing.
