...
Furthermore, the framework quantitatively prohibits causally silent dark matter
(WIMPs, axions, sterile neutrinos): a 100~GeV WIMP generates $10^{25}$--$10^{38}$
times less actualization flux than equivalent baryonic matter, falling orders of
magnitude below the threshold for measurable gravitational lensing. The conjectural
...
paper RADM~\citep{Sandeman2026RADM}. A new RA-native derivation of the
...
baryonic disk mass~\citep{BlandHawthorn2016} is used --- consistent
with $\alpha = 2\pi$ exactly within the systematic uncertainty on
$v_{\mathrm{flat}}$ and $r_d$. Four mathematical derivations remain open and are precisely scoped for collaborators.
...
the companion paper \emph{The Engine of Becoming}~\citep{Sandeman2026EB}, but
...
\paragraph{Flat rotation curves, the Bullet Cluster, and the topological boundary
mechanism.}
The same actualization-density field that produces the WIMP prohibition also
suggests mechanisms for flat galactic rotation curves (via a $\nabla^2(\ln\lambda)$
gradient correction to the metric sourcing equation) and for the Bullet Cluster
gravitational lensing offset (via the distinction between instantaneous
actualization flux $\lambda$ and accumulated causal depth $A_{\mathrm{RA}}$).
These mechanisms are conjectural and require covariant derivation before they
constitute a dark matter replacement. The covariant form of the
$\nabla^2(\ln\lambda)$ weak-field correction is $\Box(\ln\lambda)$, where
$\Box = g^{\mu\nu}\nabla_\mu\nabla_\nu$ is the covariant d'Alembertian;
demonstrating how this couples to $g_{\mu\nu}$ as a sourcing term, recovering
the weak-field Poisson equation in the appropriate limit, and establishing
Tully-Fisher mass scaling, CMB structure formation, and quantitative Bullet
Cluster lensing convergence are the open problems of
RADM~\citep{Sandeman2026RADM}. They are developed as the primary subject of the companion paper
RADM~\citep{Sandeman2026RADM}.
...
The central open problem is fixing the coupling constants $c_k$ in the RA-CSG
...
and $p_{\mathrm{th}} \approx 10^{-2}$ is the Kinematic Coherence Bound of
...
While the gradient correction $\rho_\lambda = -(\xi/4\pi G)\nabla^2(\ln \lambda)$
accurately recovers flat galactic rotation curves in the non-relativistic regime
(see RADM~\citep{Sandeman2026RADM}), establishing its covariant origin is
paramount. Building on Jacobson's thermodynamic derivation of the Einstein
...
(the unified Poisson equation of~\citep{Sandeman2026RADM}).
...
\subsubsection*{Refining Hard Wall D3: The Bianchi Constraint}

The contracted Bianchi identity requires $\nabla^\mu G_{\mu\nu} \equiv 0$.
Applying the commutator of covariant derivatives to the scalar field
$\phi = \ln\lambda$ yields:
\begin{equation}
  \nabla^\mu\!\left(\nabla_\mu\nabla_\nu\phi
    - g_{\mu\nu}\,\Box\phi\right)
  = R_{\mu\nu}\,\nabla^\mu\phi,
  \label{eq:bianchi_commutator}
\end{equation}
so that $\nabla^\mu\Theta_{\mu\nu}^{(\lambda)} = \xi
R_{\mu\nu}\nabla^\mu(\ln\lambda)$. Imposing the Bianchi identity on the
full modified field equations~\eqref{eq:RA_field_eqs} yields a derived
equation of motion for the actualization field:
\begin{equation}
  \nabla^\mu T_{\mu\nu}^{(A)}
    + \xi\, R_{\mu\nu}\,\nabla^\mu(\ln\lambda) = 0.
  \label{eq:bianchi_constraint}
\end{equation}
This is not merely a gap; it is a positive result. The actualization field
$\lambda$ is not freely specifiable: it is dynamically constrained by the
curvature it sources. Standard matter-energy satisfies
$\nabla^\mu T_{\mu\nu}^{(A)} = 0$ strictly only when the actualization
gradient is orthogonal to the Ricci tensor. In physically relevant
configurations the two are coupled, and equation~\eqref{eq:bianchi_constraint}
governs that coupling.

\textbf{Remaining hard wall.} Proving that the discrete quantum measure of the
causal DAG enforces equation~\eqref{eq:bianchi_constraint} in the continuum
limit---and that no anomalous momentum exchange survives the coarse-graining---
is the primary open mathematical challenge of Relational Actualism cosmology.
\textbf{Collaborator target:} algebraic QFT, Tomita-Takesaki modular theory,
causal set theory.
  ...
  Becoming~\citep{Sandeman2026EB} selects one actualization vertex per
  ...
  \textit{Covariant formulation target.} The weak-field correction
  $\nabla^2(\ln\lambda)$ is a Newtonian-limit expression. Its covariant
  generalisation is the d'Alembertian acting on the log-density field:
  \begin{equation}
    \Box(\ln\lambda) \;=\;
    g^{\mu\nu}\nabla_\mu\nabla_\nu(\ln\lambda),
    \label{eq:box_ln_lambda}
  \end{equation}
  where $g^{\mu\nu}$ is the emergent metric of Derivation~2. The full covariant
  coupling of this term to $g_{\mu\nu}$ --- establishing how
  $\Box(\ln\lambda)$ enters the modified Einstein tensor and what its coupling
  constant $\xi$ reduces to in the Newtonian limit --- is a named target for
  the covariant field equation program in Derivation~3. Demonstrating that
  $\Box(\ln\lambda) \to \nabla^2(\ln\lambda)$ in the static weak-field limit
  is a consistency requirement; proving Bianchi compatibility of the resulting
  $\langle T^{\mu\nu}_{RA}\rangle + \xi \, g^{\mu\nu}\Box(\ln\lambda)$
  source tensor is the associated hard wall.
  \textbf{Collaborator target:} general relativity, modified gravity.

  \item[Derivation 6 (WEP Recovery and $\lambda$ Operationalization).]
  \textit{Qualitative argument established; quantitative proof and forecast
  outstanding.} WEP recovery for ordinary matter argued via equilibrium limit
  ($\rho_A \to \rho_{\mathrm{matter}}$); WIMP non-universality reframed as
  $A_{\mathrm{RA}} \approx 0$, not a violation for equilibrium species.
  Observable proxies for $\lambda(x)$ identified (galaxy density, SFR,
  baryon density). \textbf{Hard walls:} (i) formal proof of WEP recovery with
  explicit correction bounds; (ii) quantitative $H_0$-density slope from RA
  field equation; (iii) kSZ consistency check; (iv) fifth-force/atom-interferometry
  predictions for feebly-interacting species.
  \textbf{Collaborator targets:} precision gravity, observational cosmology,
...
\bibitem[Sandeman(2026e)]{Sandeman2026EB}
J.~F.~Sandeman.
