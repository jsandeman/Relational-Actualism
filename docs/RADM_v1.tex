...
\begin{description}
  ...
  Becoming~\citep{Sandeman2026EB} fall into the same Lorentzian causal
  ...
  \item[Hard Wall 5: Weyl curvature sourcing, lensing, and the covariant
  $\Box(\ln\lambda)$ field equation.] The RA-modified field
  equations source the Ricci tensor $R_{\mu\nu}$ via $\langle T^{\mu\nu}_{RA}
  \rangle$. Gravitational lensing traces the Weyl tensor $C_{\mu\nu\rho\sigma}$
  via null geodesics. This hard wall has three components: (i)~deriving the
  covariant form of the $\nabla^2(\ln\lambda)$ gradient correction, which is
  $\Box(\ln\lambda) = g^{\mu\nu}\nabla_\mu\nabla_\nu(\ln\lambda)$ with the
  appropriate coupling to $g_{\mu\nu}$; (ii)~demonstrating how accumulated
  causal depth $\ARA$ contributes to Weyl curvature at the moment of a cluster
  collision; and (iii)~establishing that the integrated stellar trajectory over
  10~Gyr does not generate a gravitational wake --- the DAG records current
  causal depth, not historical positions, so no wake is expected, but this must
  be proved in the continuum limit. All three require the full covariant RA
  field equations. RAGC~\citep{Sandeman2026b} (thermodynamic derivation section) now establishes
  those field equations via a thermodynamic derivation, and identifies the
  precise Bianchi constraint $\nabla^\mu T_{\mu\nu}^{(A)} + \xi
  R_{\mu\nu}\nabla^\mu(\ln\lambda) = 0$ that governs the coupling between
  matter and the actualization field. The remaining gap is proving this
  constraint holds in the continuum limit of the discrete DAG. In the
  weak-field quasi-static limit, Weyl and Ricci sourcing are related by the
  tidal
  decomposition of the stress-energy, and the $10^8$ headroom in
  equation~\eqref{eq:RA_ratio} is expected to survive the covariant
  generalisation.
  \textbf{Collaborator target:} general relativity, gravitational lensing
  theory, algebraic QFT.
\end{description}
