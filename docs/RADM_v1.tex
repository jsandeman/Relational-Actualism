...
\medskip
\begin{abstract}
The companion paper RAGC establishes that causally silent
...
any new particle species. The covariant field equation
$G_{\mu\nu} = 8\pi G\,\mathcal{P}_{\mathrm{act}}[T_{\mu\nu}]$ (with $\Lambda=0$)
is now proved as a theorem in the companion paper
RACL, closing the primary hard wall of
RAGC. Derived as the static, weak-field limit of this
uniquely determined field equation, we utilise the
...
equilibrium limit) and $\Sigma_\lambda = \xi/(4\pi G R r_d)$ is the projected
surface-density correction from the 2D cylindrical Laplacian of $\ln\lambda$
for a disk geometry. We demonstrate that this term produces flat galactic rotation
curves for exponential disk profiles (calibrating $\xi = 2v_{\mathrm{flat}}^2 r_d$),
and that in the sparse halo regime DJW dimensional reduction gives an SIS-equivalent
lensing profile with Einstein radius $\theta_E \approx 0.47$~arcsec for a Milky-Way
analog ($v_{\mathrm{flat}} = 220$~km\,s$^{-1}$), rising to $0.9$--$1.4$~arcsec for
massive early-type lenses, consistent with observed strong-lensing Einstein radii. Furthermore, we formalise the distinction between
...
the exact value $\alpha = 2\pi$ for the Jacobson coefficient confirmed to within 2\% using the full baryonic disk mass, proof of dimensional reduction in the
...
Poisson-CSG generative measure of RAGC (Section~4.3 therein) via
$H_{\mathrm{eff}} = H_{\mathrm{CMB}}(1 + f_b|\delta|)$ where $f_b \approx 0.42$
is the void-filling baryon fraction---predicts
$H_{\mathrm{local}} \approx 73.1\;\mathrm{km\,s^{-1}\,Mpc^{-1}}$
