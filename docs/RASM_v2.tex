...
\paragraph{Three generations from 4D BDG spatial boundary structure.}
The BDG action in 4D counts four interval sizes: $N_1, N_2, N_3, N_4$.
The macroscopic spacetime embedding is 4-dimensional with a 3-dimensional
spatial boundary. The three non-trivial excitation levels $\{N_2, N_3, N_4\}$
--- the levels above the baseline $N_1$ structure --- correspond precisely
to the three independent directions of the 3D spatial boundary in which
BDG branching can occur. The number of generations is therefore not an
empirical input: it equals the spatial dimension $d_s = 3$, which in turn
equals the number of BDG excitation levels available in 4D above the
vacuum baseline.

This identification is a structural consequence of 4D BDG topology and constitutes
an RA-native derivation of the three-generation structure, not a phenomenological
assumption. Deriving the precise mass ratios between generations from the BDG
coefficients is the remaining open target (Table~\ref{tab:epistemic},
Derivation~2).
