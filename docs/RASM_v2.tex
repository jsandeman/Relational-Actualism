...
derives the coupling from the 4D Alexandrov coherence volume and the RA
actualization threshold condition, using only the BDG integers and standard
Minkowski geometry (no free parameters). A companion cosmological derivation
gives $\Omega_{\mathrm{CDM}}/\Omega_b = 17.32 \times \alpha_s(2m_p) = 5.40$,
consistent with Planck~2018 ($5.416$, $0.3\%$ error), providing the first
BDG-native derivation of a cosmological matter density ratio.

\textbf{Open.}
Deriving the RA renormalization group flow (the QCD running from Planck to
nuclear scales) and the Wyler formula for $\alpha_{\mathrm{EM}}$ from the
BDG path weight at $\mu\to 0$.
