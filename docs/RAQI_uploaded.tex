...
to the vacuum. The companion paper \emph{Relational Actualism: The Topology of
Matter} (RATM) has now established, via 67 Lean-verified
theorems, that the BDG-stable causal topology types in 4D correspond exactly to
the Standard Model particle spectrum --- and that electrons and photons, the
particles from which qubits are built, propagate at the \emph{minimum} BDG
actualization robustness score of~1. Qubit fragility is structurally grounded:
no stable particle with lower BDG score exists. In the perturbative regime this coincides with the on-shell
...
the companion papers RAQM, RAGC, RACI, and RAHC---for two fields that have lacked a
...
Lean-QuantumInfo library. The RA suite now has
\textbf{101 Lean-verified results} (zero \texttt{sorry} tags) across
\texttt{RA\_AQFT\_Proofs\_v2.lean} and the new \texttt{RA\_D1\_Proofs.lean},
which proves the BDG particle classification including colour confinement
and the particle universe closure theorem. The RA suite now has
\textbf{93 Lean-verified results} across two proof files
(\texttt{RA\_AQFT\_Proofs\_v2.lean}: 26 results;
\texttt{RA\_D1\_Proofs.lean}: 67 results), all with zero \texttt{sorry} tags
and no axioms beyond the Mathlib standard library. A key quantitative connection: the Kinematic Coherence Bound
threshold $p_{\mathrm{th}} = \alpha_{\mathrm{EM}} \approx 1/137 \approx 7.3\times10^{-3}$ is shown to be consistent with the
galactic-scale coupling constant $\xi$ of the companion paper
RADM to within ${\sim}2\%$ using the full baryonic disk mass, at the Planck scale, via the
thermodynamic field equations established in RAGC.
The Kinematic Coherence Bound threshold $p_{\mathrm{th}} = \alpha_{\mathrm{EM}} \approx 1/137 \approx 7.3\times10^{-3}$
...
dimensional reduction and the Tully-Fisher relation.
...
\medskip\noindent
\textbf{Keywords:} Relational Actualism \textperiodcentered{} Kinematic Snap \textperiodcentered{} BDG Particle Classification \textperiodcentered{} Qubit Fragility
...
\paragraph{Structural qubit fragility: BDG score = 1.}
The Lean-verified BDG particle classification (RATM~\citep{Sandeman2026RATM},
Theorem~D1a) establishes that every stable particle worldline has a positive BDG
score, and that the minimum positive score is~1. Electrons, photons, and neutrinos
--- the particles from which physical qubits are built --- propagate at the
sequential fixed point $N{=}(1,1,1,1)$ with score~1. This is not a coincidence:
it is the lowest score the BDG filter permits for a stable asymptotic particle.
By contrast, $W^{\pm}$ bosons have score~9 and gluons have score~18.

\begin{center}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{lcc}
\toprule
Particle & BDG topology type & Score \\
\midrule
$\nu$, $\gamma$, $Z^0$, $H$ & Sequential, isolated & 1 \\
$e^-$, $\mu^-$, $\tau^-$ & Sequential, depth-4 & 1 \\
$u$, $c$, $t$ quarks & Topological (asym-Y) & 8 \\
$W^{\pm}$ bosons & Sequential, depth-2 & 9 \\
Gluons $g$ & Topological (sym-Y) & 18 \\
\bottomrule
\end{tabular}
\end{center}

The BDG score measures the actualization robustness of a vertex environment:
a score of 18 means the gluon's causal topology is strongly self-reinforcing;
a score of~1 means the electron's environment sits at the threshold of stability.
Qubit fragility is therefore \emph{structural}: physical qubits are built from
the particles with the minimum BDG actualization robustness that the causal
graph permits. A qubit implemented in $W^\pm$ bosons would be nine times more
BDG-stable --- but $W^\pm$ bosons decay in $\sim\!10^{-25}$~s and cannot be
confined in a laboratory register. Nature provides precisely the stable
particles needed for quantum information, and those particles sit at the BDG
floor. This result is Lean-verified (theorems \texttt{D1a\_fixed\_point},
\texttt{D1b\_both\_stable}); the quantitative connection to $T_2$ timescales
requires closing Derivation~2 (the $\mu_{\mathrm{stable}}$ simulation).
...
Concretely: for a surface code with $p_{\mathrm{th}} = \alpha_{\mathrm{EM}} \approx 1/137 \approx 7.3\times10^{-3}$ under
...
$p_{\mathrm{th}} = \alpha_{\mathrm{EM}} \approx 1/137 \approx 7.3\times10^{-3}$ is not a property of specific error
...
\paragraph{Closure of decoherence channels.}
The BDG particle universe closure theorem
(RATM~\citep{Sandeman2026RATM}, Theorem~D1\_closure\_complete) proves
exhaustively --- over 124 extension cases --- that the seven-element set of
BDG N-vector classes is closed under all single-step graph extensions. Every
particle type that can couple to a qubit must belong to this closed set.
The practical implication for quantum computing is a constraint on environmental
decoherence channels: photonic coupling (sequential $\gamma$), phononic
coupling (sequential, lattice vibrations mediated by sequential-class bosons),
and magnetic coupling (sequential $Z^0/\gamma$ exchange) are all permitted;
no decoherence channel involving a particle type outside the known set is
possible within the RA framework. This rules out, in principle, exotic
decoherence mechanisms involving hypothetical particles with causal topology
outside the BDG closure. The constraint is conditional on the completeness of
D1b (proved to graph size~7; conjectured complete); if D1b is complete at all
sizes, the decoherence channel constraint is unconditional within RA.
...
The Lean-verified BDG particle classification established in the companion
paper RATM~\citep{Sandeman2026RATM} deepens the structural account of qubit
fragility. Electrons and photons --- the particles that carry quantum
information in all current hardware --- propagate at BDG score~1, the minimum
positive score the causal filter permits. This is not a property that can be
engineered away; it is a consequence of the causal topology of the only
particles stable enough to exist as free asymptotic states in the
low-energy regime. The $W^\pm$ boson at score~9 and the gluon at score~18
are more BDG-robust but inaccessible as qubit carriers at laboratory scales.
The Kinematic Coherence Bound thus has a deeper grounding: it is not merely
that electrons decohere at a certain rate, but that they decohere at the
minimum rate structurally possible for any particle that can form a
laboratory qubit. The 93 Lean-verified results across the RA suite
provide the machine-checked formal foundation for these claims.
