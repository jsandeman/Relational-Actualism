...
(BDG) causal set action in $d=4$ spacetime dimensions selects exactly
five self-similar vertex topology classes that correspond precisely to
the particle content of the Standard Model. The classification is
topological: causal vertices whose past is totally ordered (sequential)
correspond to leptons and gauge bosons propagating as free asymptotic
states; vertices whose past contains a spacelike pair (topological)
correspond to quarks and gluons. This partition, together with the
$Q_{N_1}$ winding number encoding electromagnetic charge, gives a
complete two-level derivation of the SM particle spectrum from
4D causal graph geometry alone.

Three central results are machine-verified:
\textbf{(D1a)} The BDG-stable sequential chain depths are exactly
$k \in \{0, 2, 4\}$, with $N{=}(1,1,1,1)$ a genuine fixed point proved
by definitional equality.
\textbf{(D1c)} Colour confinement is a \emph{theorem}: gluon and quark
worldlines each encounter a BDG filter event and cannot propagate
freely as asymptotic states, with finite confinement lengths $L{=}3$
(gluon) and $L{=}4$ (quark).
\textbf{D1\_closure\_complete}: The seven-element BDG N-vector set is
\emph{closed} under all single-step graph extensions~--- verified
exhaustively over 124 extension cases. No new elementary particle
topology type can emerge from any local causal move.

The Lean~4 proof file \texttt{RA\_D1\_Proofs.lean} contains 67~results
(56~theorems, 11~lemmas, 8~definitions) with zero \texttt{sorry} tags
and no axioms beyond the Mathlib standard library. Combined with the
26~results of \texttt{RA\_AQFT\_Proofs\_v2.lean}, the Relational
Actualism suite now has \textbf{93 Lean-verified results}.
This constitutes the first machine-verified particle classification
theorem in the causal set theory literature.

Additional results: the gluon is strictly more confined than the quark
(gluon extensions are either self-replicas or filtered; quarks have
escape routes to sequential character); the boundary between
propagation vertices and interaction vertices is precisely characterised;
the Gell-Mann--Nishijima formula is verified for all six first-generation
fermions in BDG language; and the coherent state Clebsch-Gordan ratio
$|\hat{\phi}(n{=}1)|^2/|\hat{\phi}(n{=}0)|^2 = 1/2$ connects the
Lean-proved $SU(3)_{\mathrm{gen}}$ coherent state to the
isospin doublet structure.

\medskip
\noindent\textbf{Keywords:} Causal set theory $\cdot$ Benincasa-Dowker action
$\cdot$ Particle classification $\cdot$ Colour confinement $\cdot$
Lean~4 formal verification $\cdot$ Relational Actualism $\cdot$ Standard Model
$\cdot$ Lepton-quark distinction $\cdot$ Causal graph topology
\end{abstract}
...
In Lean~4:
\begin{verbatim}
def bdgScore (N1 N2 N3 N4 : \mathbb{Z}) : \mathbb{Z} :=
...
\label{thm:closure}
The seven-element N-vector set
\[
...
in $\mathcal{U}$. Verified exhaustively:
\begin{itemize}[noitemsep]
...
RASM~\citep{Sandeman2026RASM}. The BDG classification is the
...
(in \texttt{RA\_AQFT\_Proofs\_v2.lean}) establishes:
\[
...
constant problems~\citep{Sorkin2012} and path integral
...
This paper is part of the nine-paper Relational Actualism suite.
...
\texttt{RA\_D1\_Proofs.lean} contains 67 results (56 theorems, 11 lemmas)
...
All Lean~4 proof files and Python verification scripts are available
at \url{https://github.com/jsandeman/Relational-Actualism}.
...
\bibitem[Sorkin(2012)]{Sorkin2012}
R.~D. Sorkin.
