...
three are in peer review (\emph{Foundations of Physics} — Accept with
Minor Revisions; \emph{Classical and Quantum Gravity}; \emph{Physical Review D}).
...
New results (April 2026): the wave-function actualization threshold
$\Delta S^* = -\log P_{\mathrm{acc}}(1) \approx 0.601$~nats is derived
from the BDG Poisson-CSG at $\mu = 1$, extending the four-scale to a
five-scale $\mu = 1$ unification; a spin-bath worked model yields the
parameter-free prediction $t^* \approx 0.274/g$; and a Lean theorem
$\to$ physics claim correspondence table (Table~2 of RAQM) maps seven
Lean-verified results to their physical claims.
RAQM has received an Accept with Minor Revisions recommendation from
\emph{Foundations of Physics} (April 2026).
\end{abstract}
...
\begin{itemize}[noitemsep]
  \item RAQM at \emph{Foundations of Physics}: Accept with Minor Revisions
  (second round, April 2026).
  RACL submitted to \emph{Classical and Quantum Gravity};
  RATM submitted to \emph{Physical Review D}.
...
\newblock \emph{Foundations of Physics} (Accept with Minor Revisions), 2026.
