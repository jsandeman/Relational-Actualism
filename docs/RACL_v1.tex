...
\begin{center}
  ...
  \vspace{8pt}
  {\small\textit{Preprint:}~\href{https://doi.org/10.5281/zenodo.19270020}{doi.org/10.5281/zenodo.19270020}\par}
  \vspace{14pt}
\end{center}
...
The derivation assembles four ingredients: the Lean~4-verified
Local Ledger Condition (LLC); a new theorem showing
$\nabla_\mu(\Pact T^{\mu\nu}) = 0$ for the actualization-projected
stress tensor; the Benincasa-Dowker theorem~\citep{Benincasa2010} that
$\langle\SBDG\rangle \to (l_P^2/4)R$ in the dense-graph limit; and
Lovelock's uniqueness theorem~\citep{Lovelock1971}. Vacuum energy
suppression~\citep{Sandeman2026RAGC}, proved separately via the
actualization projector $\Pact$, eliminates the cosmological constant
leaving no free parameter. The Bianchi identity $\nabla_\mu G^{\mu\nu}
\equiv 0$ holds by geometry, not dynamics.

Two results are new. First, a \emph{BDG locality lemma}: at Poisson
vertex density $\lambda$, the BDG score at vertex $v$ depends only
on ancestors within proper-time window
$\tau_{\mathrm{BDG}}(\lambda) = 4\,\Gamma(5/4)\,(12/\pi^2)^{1/4}\,
\lambda^{-1/4} \to 0$ as $\lambda \to \infty$, confirming locality
of the continuum limit. Second, the \emph{P$_{\mathrm{act}}$
conservation theorem}: $\nabla_\mu(\Pact T^{\mu\nu}) = 0$, proved via
a Leibniz decomposition using the LLC, the vacuum energy suppression
theorem, and the Lean-verified frame-independence of the relative
entropy $\Delta S(\rho\|\sigma_0)$.

The proof is RA-native: it does not take GR as a target to recover
from a discrete structure. Rather, it shows that the LLC---a
vertex-level charge balance condition, verified in Lean~4 with zero
\texttt{sorry} tags---forces the macroscopic field equation to be
the Einstein equation. GR is an output of RA, not an input.

Where RA departs from GR is precisely where the actualization
projector $\Pact T^{\mu\nu}$ departs from $T^{\mu\nu}$: in sparse
regions where the on-shell fraction $\lambda_{\rm local}/\lambda_{\rm
ref} < 1$. This departure \emph{is} the Hubble tension and galactic
rotation curves. The RA corrections are not quantum-gravitational Planck-scale
effects; they are classical actualization-density gradient effects at
cosmological and galactic scales.

\medskip
\noindent\textbf{Keywords:} Relational Actualism $\cdot$
Einstein field equations $\cdot$ Local Ledger Condition $\cdot$
Lean~4 formal verification $\cdot$ Lovelock theorem $\cdot$
Benincasa-Dowker action $\cdot$ Actualization projector $\cdot$
Vacuum energy $\cdot$ BDG locality $\cdot$ Causal set theory
\end{abstract}
...
\texttt{RA\_AQFT\_Proofs\_v2.lean}. The Graph Cut
...
\texttt{frame\_independence} in \texttt{RA\_AQFT\_Proofs\_v2.lean},
...
increase in $\lambda$, the ratio $\tau_{\mathrm{hop}}(\lambda) /
\tau_{\mathrm{hop}}(10\lambda) = 10^{1/4} = 0.5623\ldots$, matching
...
in \texttt{RA\_AQFT\_Proofs\_v2.lean} is what makes this step rigorous.
\end{remark}
...
\begin{itemize}[noitemsep]
  \item \textbf{Galactic rotation curves}: the exponential stellar
    disk gives $\nabla^2\ln\lambda = -2/(r\cdot r_d)$, producing a
    $1/r$ effective density that yields perfectly flat rotation curves.
    Numerical verification confirms flatness to 4.3\% for $r > 5$~kpc
    in a Milky Way analog~\citep{Sandeman2026RADM}.
...
\midrule
LLC (Definition~\ref{def:LLC}) & \texttt{RA\_AQFT\_Proofs\_v2.lean} & Lean-proved, 0 sorry \\
Graph Cut Theorem & \texttt{RA\_AQFT\_Proofs\_v2.lean} & Lean-proved, 0 sorry \\
Frame independence of $\dS$ & \texttt{RA\_AQFT\_Proofs\_v2.lean} & Lean-proved, 0 sorry \\
...
across \texttt{RA\_AQFT\_Proofs\_v2.lean} (26 results) and
...
derived field equations has been noted before (see e.g.~\citep{Padmanabhan2013}).
...
in Padmanabhan's emergent gravity programme~\citep{Padmanabhan2013},
...
All Lean~4 proof files, Python verification scripts, and the BDG
locality computation are available at
...
\bibitem[Padmanabhan(2013)]{Padmanabhan2013}
T.~Padmanabhan.
