...
three are in peer review (\emph{Foundations of Physics}; \emph{Classical
and Quantum Gravity}).
...
The single highest-value remaining computation: running the BDG-filtered
Poisson-CSG simulation to its stationary distribution to determine the
asymptotic $c_k$ profile.  This simultaneously gives the QCD string tension
$\sigma$, the Fermi constant $G_F$, the Hubble bandwidth ratio
$\bar\lambda_m/\bar\lambda_{\mathrm{exp}}$, and a test of the Wyler formula
for $\alpha_{\mathrm{EM}}$.  \textbf{Note:} $\alpha_s(m_Z) = 1/\!\sqrt{c_2 c_4}$
is now \emph{proved} independently from the BDG path weight at $\mu=1$
(ra\_bdg\_couplings~\citep{Sandeman2026BDG}), so the coupling constant
target of D1 reduces to $\alpha_{\mathrm{EM}}$ and $G_F$.  Closing D1 would
still derive all fermion masses from
...
\begin{itemize}[noitemsep]
  \item Three papers in peer review: RAQM at \emph{Foundations of Physics}
  (past desk rejection, in editor assignment); RACL submitted to
  \emph{Classical and Quantum Gravity}; RATM submitted to
  ...
  \item 101 Lean-verified results across two proof files, zero
  \texttt{sorry} tags.
  \item 52/52 independent numerical checks at machine precision.
  \item Three open derivation targets (D1--D3) with precise
  success criteria and identified collaborator communities.
\end{itemize}
