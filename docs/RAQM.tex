...
(RAQM), an ontological framework in which wave function actualization is grounded
in physical interaction rather than observation. The actualization criterion is:
an interaction constitutes an actualization event when it produces an irreversible
increase in quantum relative entropy, $\Delta S(\rho\|\sigma_0) > 0$, generating
a robust causal record. In perturbative QFT, on-shell boson emission is a
sufficient implementation of this criterion; the entropic condition is fundamental
and does not require sharp spacetime localization. The actualization map is defined
as a localized quantum instrument whose Kraus operators act on the causal diamond
of the interaction. We derive a consistency condition showing that the non-selective
actualization map preserves observable statistics at spacelike-separated points
regardless of hypersurface choice---a no-signaling result following from algebraic
microcausality, not postulated. A spin-bath worked model yields a parameter-free
actualization timescale $t^* = (1/g)\sqrt{\Delta S^*/2}$, where $\Delta S^*$ is
derived from the causal graph dynamics (companion paper RASM). Three discriminating
predictions are stated, the sharpest being a reversibility threshold testable via
Loschmidt echo protocols. RAQM is distinguished from Relational QM, dynamical
collapse models, decoherence-only approaches, and Many-Worlds.

\medskip\noindent
\textbf{Keywords:} Relational Quantum Mechanics \textperiodcentered{} Wave
Function Collapse \textperiodcentered{} Actualization Criterion
\textperiodcentered{} Quantum Relative Entropy \textperiodcentered{}
Microcausality \textperiodcentered{} No-Signaling \textperiodcentered{}
Decoherence \textperiodcentered{} Causal DAG
\end{abstract}
...
defined via the Araki formula for type III$_1$ von Neumann factors. In
RA, the finite-dimensional Lean-verified results are the fundamental
description---the discrete causal graph is the primitive reality, and
type~III$_1$ algebras are the continuum approximation that emerges at
$\mu \gg 1$. The Lean~4-verified results establish the physics at the
discrete level; the type~III$_1$ framework is what these results
\emph{look like} when coarse-grained into continuum AQFT. Showing
compatibility is a consistency check, not a foundational requirement
(the same reframing that dissolved the discrete Bianchi and Lorentz
emergence problems---see RAGC).
...
algebras but is distinct from strict continuum QFT. RA treats the
finite-dimensional representation as fundamental (the universe is
discrete at the Planck scale); the continuum type~III$_1$ algebra is
the macroscopic approximation, not the other way around.
...
The coupling constants of the Standard Model are derived from the BDG
integer structure in the companion paper RASM~\cite{Sandeman2026RASM};
they are not free parameters of the framework.

\begin{definition}[The Actualization Map]
\label{def:actualization_map}
Let $\mathcal{A}(\mathcal{O})$ be the local algebra of observables
associated with a causal diamond $\mathcal{O}$ containing the interaction
vertex. An actualization event at $\mathcal{O}$ is described by a
\emph{quantum instrument} $\{\inst_k\}_{k=1}^K$: a collection of
completely positive (CP) maps $\inst_k(\rho) = M_k \rho M_k^\dagger$
satisfying the completeness relation
$\sum_k M_k^\dagger M_k = I$
(guaranteeing that the non-selective map
$\mathcal{A}(\rho) = \sum_k \inst_k(\rho)$ is CPTP). The Kraus operators
$M_k$ act on $\mathcal{H}_{\mathcal{O}}$ and are determined by the
physical interaction: each $M_k$ corresponds to a distinct environmental
record state $|e_k\rangle$ such that $\langle e_j | e_k \rangle
= \delta_{jk}$ (record orthogonality). Outcome $k$ occurs with
probability $p_k = \operatorname{Tr}[M_k \rho M_k^\dagger]$.

The actualization \emph{condition} is: the instrument $\{\inst_k\}$ is
applied (i.e., an actualization event occurs) if and only if the
interaction produces $\Delta S(\rho_{\mathrm{post}}\|\sigma_0) > 0$
for the post-interaction system-environment state. When this condition
is not met (purely virtual exchange), the evolution remains unitary and
no instrument is applied.
\end{definition}

\noindent
This definition makes RAQM's update rule precise: the $M_k$ are not
postulated \textit{ad hoc} but are determined by the physical interaction
Hamiltonian $H_{SE}$ restricted to the causal diamond. In the spin-bath
model of Section~\ref{sec:spinbath}, the $M_k$ are explicitly constructed
and the actualization condition $\Delta S \geq \Delta S^*$ yields the
parameter-free timescale $t^*$.
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Irreversibility is entropic, with a sharp perturbative
  implementation.} The fundamental criterion is $\Delta S(\rho\|\sigma_0) > 0$
  (Definition~\ref{def:actualization_map}). In the perturbative regime, this
  reduces to the dispersion relation $p^{\mu}p_{\mu} = m^{2}c^{2}$---a binary
  condition that acts as a sharp sufficient trigger. In strongly coupled or
  many-body regimes, the entropic criterion applies directly without requiring
  identifiable on-shell quanta.
...
\begin{enumerate}[leftmargin=*]
  ...
  By the amplitude\_locality theorem (each local amplitude depends only on
  ...
  library~\citep{Meiburg2025LQI}. These finite-dimensional results are
  the fundamental physics: the discrete causal graph is RA's primitive
  reality. The type~III$_1$ AQFT formulation is the continuum
  approximation; showing compatibility is a consistency check, not a
  requirement for the discrete results to hold.
\end{enumerate}

The non-perturbative regime is resolved in finite dimensions---which,
in RA's ontology, IS the fundamental level. Both frame independence and
...
The type~III$_1$ AQFT framework is the continuum approximation to these
discrete results; compatibility is expected but not load-bearing.
...
the companion paper RASM~\citep{Sandeman2026RASM}). The per-vertex
relative entropy produced at an actualization event at this density is:
\begin{equation}
...
\paragraph{Prediction 3: Reversibility threshold via Loschmidt echo.}
\label{pred:P3}
RAQM's most distinctive prediction. Consider a central spin coupled to
$N$ bath spins with coupling $g$ (Section~\ref{sec:spinbath}). Prepare the
system in a superposition, let it evolve for time $t$, then apply a
perfect spin-echo (time-reversal) pulse and evolve for time $t$ again.
The Loschmidt echo fidelity is
$\mathcal{F}(t) = |\langle\psi_0|\psi(2t)\rangle|^2$.

\emph{RAQM prediction:} $\mathcal{F}(t)$ remains near unity for
$t < t^*/2$ and drops abruptly to near zero at $t = t^*/2$, where
$t^* = (1/g)\sqrt{\Delta S^*/2} \approx 0.548/g$. The threshold is
sharp because it marks the transition from reversible potentia to
irreversible actuality---the permanent causal record cannot be undone
by any unitary operation.

\emph{Decoherence-only prediction:} $\mathcal{F}(t)$ decays smoothly
as $e^{-\gamma t}$ with no sharp threshold; approximate echo revival
is possible at all times with sufficiently precise control.

\emph{GRW/CSL prediction:} $\mathcal{F}(t)$ decays at a rate set by
$\lambda_{\mathrm{GRW}}$, independent of $g$; the threshold does not
scale with environmental coupling.

\emph{Everett prediction:} No sharp threshold; $\mathcal{F}(t)$ decays
smoothly and echo revival is always possible in principle (no genuine
irreversibility, only practical decoherence).

\emph{Proposed protocol:} Superconducting transmon qubit coupled to a
tunable bath of $N \sim 5$--$20$ lossy resonators. Vary $g$ by tuning
the resonator coupling capacitance. Measure $\mathcal{F}(t)$ as a
function of $t/t^*$ for at least three values of $g$ spanning an order
of magnitude. RAQM predicts: (i) a sharp drop in $\mathcal{F}$ at
$t \approx t^*/2$, (ii) the threshold time $t^*$ scales as $1/g$,
and (iii) the critical $\Delta S^*$ is the same for all $g$
(approximately $0.601$~nats). If the drop is smooth, scales with
$\lambda_{\mathrm{GRW}}$ rather than $g$, or shows no threshold,
RAQM is falsified.

\paragraph{Distinguishing RAQM from competing interpretations.}
Prediction~3 (Loschmidt echo threshold) is the cleanest discriminator.
The four competing frameworks make qualitatively different predictions
for $\mathcal{F}(t)$ as a function of $t/t^*$: RAQM predicts a sharp
threshold scaling as $1/g$; GRW/CSL predicts a threshold scaling as
$1/\lambda_{\mathrm{GRW}}$ (independent of $g$); decoherence-only
predicts smooth decay with no threshold; Everett predicts no genuine
irreversibility. Predictions~1 and~2 provide supporting tests:
propagation delay (P1) distinguishes RAQM from instantaneous collapse;
environment-dependent duration (P2) distinguishes RAQM from GRW/CSL's
fixed stochastic rate.
...
The formal verification of RAQM's core mathematical claims is summarised in
Table~\ref{tab:lean_map}. Companion papers extend the framework to gravity
and cosmology (RAGC~\citep{Sandeman2026RAGC}), particle topology
(RATM~\citep{Sandeman2026RATM}), the Standard Model spectrum
(RASM~\citep{Sandeman2026RASM}), and biological complexity
(RACF~\citep{Sandeman2026RACF}).
...
Table~\ref{tab:lean_map} maps the Lean-verified theorems that directly
support the claims of this paper.

\begin{table}[ht]
\centering
\small
\caption{Lean theorem to physics claim correspondence.
  All listed theorems compile in Lean~4 with zero \texttt{sorry} tags.}
\label{tab:lean_map}
...
whether $\Delta S > 0$. Supports the no-signaling argument
(\S\ref{sec:relativity}). \\[4pt]
...
Resolves the Unruh paradox (\S\ref{sec:criterion}). \\[4pt]

\texttt{local\_ledger\_condition} \newline
$\sum_k M_k^\dagger M_k = I$ on the DAG & 
Conservation of probability at every actualization vertex.
Grounds the CPTP structure of the actualization map
(Definition~\ref{def:actualization_map}). \\[4pt]

\texttt{graph\_cut\_theorem} \newline
LLC holds independently on each subgraph at causal severances &
Conservation is locally valid at event horizons; supports the
causal DAG structure. \\[4pt]

\texttt{bdg\_amplitude\_locality} \newline
$a(v|C) = a(v|C')$ when $C \cap \mathrm{past}(v) = C' \cap \mathrm{past}(v)$ &
BDG amplitude depends only on causal past. Proved as a theorem
of discrete DAG dynamics (zero sorry). \\[4pt]

\texttt{bdg\_causal\_invariance} \newline
Quantum measure permutation-invariant &
Actualization probabilities are foliation-independent. Unconditional
(zero sorry). Supports \S\ref{sec:consistency}. \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Note added in revision.}
Since initial submission, amplitude locality (O01) has been proved as a
theorem of discrete DAG dynamics, making the causal invariance result
unconditional with zero sorry tags. The actualization threshold
$\Delta S^* \approx 0.601$~nats is now derived from the BDG integers
$(1,-1,9,-16,8)$ via the Poisson-CSG at $\mu = 1$
(RASM~\cite{Sandeman2026RASM}), confirming that no free parameters
enter the actualization criterion.
...
(\texttt{RA\_D1\_Proofs.lean}, \texttt{RA\_AmpLocality.lean},
\texttt{RA\_GraphCore.lean}, \texttt{RA\_Koide.lean},
\texttt{RA\_AQFT\_Proofs\_v10.lean}, \texttt{RA\_Spin2\_Macro.lean},
\texttt{RA\_SteinChen.lean}),
