...
\$H_{\mathrm{local}} \approx 73.6$~km\,s$^{-1}$\,Mpc$^{-1}$ from the KBC
void. New falsifiable prediction: the Eridanus supervoid
($\delta \approx -0.30$) should give $H \approx 76.8$~km\,s$^{-1}$\,Mpc$^{-1}$.

Two hard walls remain fully open (quantitative Bullet Cluster lensing;
Weyl curvature sourcing); three others are substantially resolved
following results in RAGC and RASM. All are precisely scoped for collaborators.
...
Both mechanisms are conjectural in their quantitative details.
The hard walls identified in Section~\ref{sec:hardwalls} are partially
resolved following results in RAGC and RASM (April 2026); the remaining
open targets are specific computations, not conceptual gaps.
...
consistent macroscopic description (by BDG uniqueness and
Lovelock's theorem~\citep{Sandeman2026b}).
...
therefore grounded in a first-principles covariant framework. The microscopic
derivation of $\xi$ from actualization entropy (Hard Wall~1,
Section~\ref{sec:hardwalls}) is substantially resolved: $\alpha = 2\pi$
is confirmed to within $1.7\%$ using the full baryonic disk mass census.
...
\begin{description}
  \item[Hard Wall 1: Microscopic Derivation of $\xi$ --- Status: Substantially Resolved.]
  ...
  \item[Hard Wall 2: Formal Proof of Dimensional Reduction at $\lambda_c$ --- Status: Partially Resolved.]
  ...
  Hard Wall~2 is now precisely: verify that the RA-CSG dynamics
  fall into the Lorentzian causal triangulations universality class
  (the coupling constants are uniquely fixed by O14; no MCMC is needed
  for that step). The empirical MOND acceleration constant
  ...
  The coupling constants $c_k$ are uniquely determined by the O14
  uniqueness theorem (RAGC); the remaining computation is the
  spectral dimension as a function of $\mu$. Testable against
  ...
  \item[Hard Wall 4: CMB acoustic structure --- Status: Substantially Resolved.]
  ...
  \item[Hard Wall 5: Weyl curvature sourcing --- Status: Item (i) resolved; items (ii)--(iii) open.] Gravitational lensing traces the Weyl tensor
