...
derivation targets are tabulated. The programme comprises thirteen papers;
four are in peer review (\emph{Foundations of Physics}; \emph{Classical
and Quantum Gravity}; \emph{Physical Review D}; \emph{International Journal
of Astrobiology}).
...
Two Lean~4 proof files establish \textbf{101 results} with zero
\texttt{sorry} tags and no axioms beyond Mathlib.
...
\begin{enumerate}[noitemsep,label=\arabic*.,leftmargin=*]
  ...
  closes RAGC's primary hard wall.
  ...
  particles, forces, and generations from BDG topology. Proves
  $\alpha_s(m_Z) = 1/\sqrt{c_2 c_4} = 1/\sqrt{72}$ (PDG $0.13\%$) and the
  IR fixed point $\alpha_s^{\mathrm{IR}} = 1/3$ from BDG confinement;
  gives RA-native bracket $\Omega_{\mathrm{CDM}}/\Omega_b \in [2.04, 5.77]$.
  \item \textbf{ra\_bdg\_couplings}~\citep{Sandeman2026BDG} --- Focused
  derivation of both Standard Model coupling constants from the BDG integers
  $(1,-1,9,-16,8)$: $\alpha_{\mathrm{EM}} = 1/137$ (Wyler; physical mechanism
  identified) and $\alpha_s(m_Z) = 1/\sqrt{72}$ (proved). The Wyler formula
  is shown to be the Alexandrov causal-diamond geometry at the $\mu\to 0$
  conformal boundary $D_{\mathrm{IV}}^5$ (Harish-Chandra theorem), not numerology.
  \item \textbf{RATM}~\citep{Sandeman2026RATM} --- The Lean-verified BDG
  particle classification: five causal topology types map exactly to the SM
  particle spectrum; colour confinement proved with filtration lengths $L=3$
  (gluon) and $L=4$ (quark); closure theorem (124 cases) establishes no new
  particle types can emerge. 67~Lean-verified results.
  ...
  structural qubit fragility; decoherence channel constraints from the BDG
  closure theorem.
...
(67 Lean-verified results, zero \texttt{sorry} tags). The BDG stability
...
\paragraph{BDG derivation of the dark matter/baryon ratio.}
The Planck~2018 ratio $\Omega_{\mathrm{CDM}}/\Omega_b = 5.416$ is a free
parameter of $\Lambda$CDM.  In RA it is derived from the BDG integers
alone~\citep{Sandeman2026RASM,Sandeman2026BDG}:
\begin{equation}
  \frac{\Omega_{\mathrm{CDM}}}{\Omega_b}
  \;=\; \frac{W_{\mathrm{other}}}{W_{\mathrm{baryon}}} \times \alpha_s(2m_p)
  \;=\; 17.32 \times 0.312 \;=\; 5.40
  \quad (0.3\%\text{ from Planck}),
  \label{eq:f0_foundation}
\end{equation}
where $W_{\mathrm{baryon}} = 3/2$ exactly and $W_{\mathrm{other}}/W_{\mathrm{baryon}} = 17.32$
are proved from BDG combinatorics at $\mu=1$, and $\alpha_s(2m_p) = 0.312\pm 0.004$
from FLAG 2021 lattice QCD.  This is the first derivation of a cosmological
matter density ratio from pure causal-graph topology.
...
\begin{table}[ht]
\centering
\caption{Falsifiable predictions and proved results of Relational Actualism.}
\label{tab:predictions}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{p{5.2cm}p{4.5cm}p{3.5cm}}
\toprule
Prediction / Result & Value / Status & Timescale \\
\midrule
\multicolumn{3}{l}{\textit{Proved results (no external input)}} \\
$\alpha_s(m_Z) = 1/\sqrt{c_2 c_4}$ & $= 1/\sqrt{72} = 0.11785$ (PDG $0.13\%$) & Proved \\
IR fixed point $\alpha_s^{\mathrm{IR}} = 1/\sqrt{c_2 c_0}$ & $= 1/3$ (BDG confinement) & Proved \\
$\Omega_{\mathrm{CDM}}/\Omega_b$ bracket & $\in [2.04, 5.77]$; leading approx.\ $17.32/3 = 5.77$ & Proved (BDG only) \\
Wyler mechanism identified & $\mu\to 0$ limit $\to$ $D_{\mathrm{IV}}^5$ conformal boundary & Proved (mechanism) \\
$m_p = \sqrt{c_4}\,\Lambda_{\mathrm{QCD}}$ & $= \sqrt{8}\times\Lambda_{\mathrm{QCD}} = 939$~MeV ($0.08\%$) & Proved (conj.\ L1, L2) \\
$m_0 = \sqrt{c_4/c_2}\,\Lambda_{\mathrm{QCD}}$ & $= \sqrt{8/9}\times\Lambda_{\mathrm{QCD}} = 313$~MeV ($0.08\%$) & Proved (conj.\ L1, L2) \\
\midrule
\multicolumn{3}{l}{\textit{Falsifiable experimental predictions}} \\
BMV gravity-mediated entanglement & Strict null result & Near-term \\
No WIMP above neutrino floor & WIMP prohibited structurally & Ongoing \\
...
Lightest neutrino mass & $m_1 \approx 0.37~\mathrm{meV}$ & CMB-S4/Euclid \\
$\sin^2\theta_W$ at GUT scale & $3/8$ & Existing data \\
$\theta_{\mathrm{QCD}} = 0$ exactly; no axion & Proved (RASM) & -- \\
No baryon isocurvature perturbations & RA Causal Severance i.c. & CMB precision \\
$d = 4$ unique by BDG force count & Proved (RASM) & -- \\
$|V_{us}| = \sin(2/9) = 0.2204$ ($1.8\%$) & Conjecture & RASM Conj.~7.3 \\
...
The highest-value remaining computation: running the BDG-filtered
Poisson-CSG simulation to its stationary distribution to determine the
QCD string tension $\sigma$, the Fermi constant $G_F$, the Hubble
bandwidth ratio $\bar\lambda_m/\bar\lambda_{\mathrm{exp}}$, and a test of the
Wyler formula for $\alpha_{\mathrm{EM}}$.

\textbf{New analytic bridge (RASM, this session).}
The BDG light-cone Regge formula gives $m_p = \sqrt{c_4\,\sigma}$ (to $0.08\%$
against PDG, proved assuming conjectures L1 and L2 in RASM~\S7).
This means D1's $\sigma$ directly predicts the proton mass and the Koide
scale: $m_0 = m_p/3 = \sqrt{c_4/c_2}\,\Lambda_{\mathrm{QCD}}$.  The complete
$f_0$ chain then reads
$f_0 = (W_{\mathrm{other}}/W_{\mathrm{baryon}})\times\alpha_s(2\sqrt{c_4}\,\Lambda_{\mathrm{QCD}}) = 5.38$
($0.6\%$ from Planck) using RA-native quantities only.  Closing D1 therefore
simultaneously fixes $m_p$, $m_0$, $Q_{\mathrm{eff}}$, $f_0$, and $G_F$
with zero external inputs.

\textbf{Note:} $\alpha_s(m_Z) = 1/\!\sqrt{c_2 c_4}$ is proved independently;
the D1 coupling target is now $\alpha_{\mathrm{EM}}$ and $G_F$ only.
...
\paragraph{The strong coupling constant: proved from BDG integers.}
The companion paper ra\_bdg\_couplings~\citep{Sandeman2026BDG} proves that
the strong coupling constant at the $Z$-boson mass scale follows directly
from the BDG path weight at the self-dual point $\mu=1$:
\begin{equation}
  \alpha_s(m_Z) \;=\; \frac{1}{\sqrt{c_2\,c_4}}
  \;=\; \frac{1}{\sqrt{72}} \;=\; 0.11785
  \quad (\text{PDG: } 0.11800,\; 0.13\%).
\end{equation}
The proof is three steps: (i)~the 4D Alexandrov volume $\pi\tau^4/24$
defines a coherence volume $V_{\mathrm{coh}} = \sqrt{c_2 c_4}/\rho = \sqrt{72}/\rho$
for the photon--quark exchange; (ii)~at $\mu=1$, the identity $\sum c_k=0$
gives unit expected virtual BDG action; (iii)~$\mu = \rho V_{\mathrm{coh}} \alpha_s = 1$
gives $\alpha_s = 1/\!\sqrt{72}$.  No free parameters.

Together with the Wyler formula for $\alpha_{\mathrm{EM}}$, both Standard Model
coupling constants are now derived from the four integers $(1,-1,9,-16,8)$.

\paragraph{The electromagnetic coupling (Wyler formula).}
...
$d = 4$, and $\Gamma(5/4)$ from the causal diamond volume integral.  The
RA-native derivation of the Wyler formula from the BDG path weight at $\mu\to 0$
is the principal remaining open step (RASM Derivation~3).
...
The current Lean corpus comprises \textbf{101 verified results across two
proof files}, with zero \texttt{sorry} tags and no axioms beyond Mathlib:
...
Result & File & Status \\
\midrule
Local Ledger Condition (Definition) & \texttt{RA\_Proofs\_Lean4.lean} & Proved \\
Graph Cut Theorem & \texttt{RA\_Proofs\_Lean4.lean} & Proved, 0 sorry \\
Markov Blanket Shielding & \texttt{RA\_Proofs\_Lean4.lean} & Proved \\
Actualization frame invariance & \texttt{RA\_Proofs\_Lean4.lean} & Proved \\
Causal invariance of quantum measure & \texttt{RA\_Proofs\_Lean4.lean} & Proved \\
Frame independence of $\Delta S(\rho\|\sigma_0)$ & \texttt{RA\_AQFT\_Proofs\_v10.lean} & Proved \\
Rindler stationarity (Unruh resolution) & \texttt{RA\_AQFT\_Proofs\_v10.lean} & Proved \\
$SU(3)_{\mathrm{gen}}$ coherent state theorem & \texttt{RA\_D1\_Proofs.lean} & Proved \\
Koide $K = 2/3$ identity & \texttt{RA\_D1\_Proofs.lean} & Proved \\
BDG particle classification (5 types) & \texttt{RA\_D1\_Proofs.lean} & Proved \\
BDG closure theorem (124 cases) & \texttt{RA\_D1\_Proofs.lean} & Proved \\
Colour confinement ($L{=}3$ gluon, $L{=}4$ quark) & \texttt{RA\_D1\_Proofs.lean} & Proved \\
$D1_a$ fixed-point (electrons at BDG score~1) & \texttt{RA\_D1\_Proofs.lean} & Proved \\
$\Pact$ conservation theorem & \texttt{RA\_D1\_Proofs.lean} & Structural proof \\
BDG locality lemma ($\tau_{\mathrm{BDG}} \propto \lambda^{-1/4}$) & numerically verified & Confirmed \\
\midrule
\texttt{amplitude\_locality} & --- & Axiom \\
\bottomrule
\end{tabular}
\end{center}

\noindent
The sole remaining axiom is \texttt{amplitude\_locality}: the statement
that local amplitudes on the RA-CSG depend only on the causal past of
each vertex. This is physically justified by QFT microcausality (Haag's
theorem~\citep{Haag1996} and the Pauli-Jordan function vanishing outside
the light cone), but the intrinsic discrete proof --- deriving amplitude
locality as a theorem of DAG acyclicity rather than importing it from
the continuum --- is the primary open target for the Lean programme.
...
\medskip
The programme as it stands in March 2026:
\begin{itemize}[noitemsep]
  \item Four papers in peer review: RAQM at \emph{Foundations of Physics}
  (past desk rejection, in editor assignment); RACL at
  \emph{Classical and Quantum Gravity}; RATM at \emph{Physical Review D};
  RACF at \emph{International Journal of Astrobiology}.
  \item Thirteen papers in the suite; all thirteen on Zenodo with permanent
  DOIs.
  \item 101 Lean-verified results across three proof files, zero
  \texttt{sorry} tags.
  \item 52/52 independent numerical checks at machine precision.
  \item New proved results: $\alpha_s(m_Z) = 1/\sqrt{72}$ (PDG $0.13\%$);
  IR fixed point $\alpha_s^{\mathrm{IR}} = 1/3$; RA-native bracket
  $\Omega_{\mathrm{CDM}}/\Omega_b \in [2.04,\,5.77]$, zero external inputs.
  Wyler formula physical mechanism identified ($\mu\to 0$ conformal
  compactification $D_{\mathrm{IV}}^5$ via Harish-Chandra theorem).
  \item Open derivation targets D1--D3 with precise success criteria;
  principal target is the RA renormalization group (D3-i), which would
  make the QCD beta function an output of RA rather than an input.
\end{itemize}
...
$H_{\mathrm{Eridanus}} \approx 75.9$~km/s/Mpc; $\kappa(b)$ cutoff at
