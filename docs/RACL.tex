...
We additionally present a second, fully RA-native derivation that
does not require Lovelock's theorem: three Lean-verified results---the
LLC~(L01), the amplitude locality theorem~(O01), and the $d=4$ BDG
closure~(L11)---combined with the published Benincasa-Dowker continuum
limit theorem, yield the Einstein-Hilbert action directly. The Bianchi
identity in this chain is the LLC viewed at the metric level, not a
separate geometric constraint.
...
\texttt{RA\_GraphCore.lean}. The Graph Cut
Theorem is proved in the same file; it shows the LLC holds independently on
each side of a causal severance, with no \texttt{sorry} in the Lean
proof.

The amplitude locality theorem---that the BDG amplitude depends only on the
causal past of the proposed vertex---has been proved as a theorem of
discrete DAG dynamics (\texttt{bdg\_amplitude\_locality} in
\texttt{RA\_AmpLocality.lean}, zero sorry tags), closing O01. Combined with
L01~(LLC) and L11~($d=4$ closure, \texttt{RA\_D1\_Proofs.lean}), these
three Lean-verified results constitute the complete discrete input to the
BDG uniqueness derivation presented in Section~\ref{sec:ra_native}.
...
in \texttt{RA\_AQFT\_Proofs\_v10.lean} is what makes this step rigorous.
\end{remark}
...
\subsection{Alternative: The RA-native derivation via BDG uniqueness}
\label{sec:ra_native}

The Lovelock derivation of Theorem~\ref{thm:main} uses differential
geometry (Lovelock's uniqueness theorem) to constrain the macroscopic
field equation. An alternative derivation, fully internal to RA, arrives
at the same result using only discrete ingredients and one published
theorem:

\begin{enumerate}[nosep]
\item \textbf{L01} (LLC, Lean-verified): local conservation at every DAG
  vertex.
\item \textbf{O01} (amplitude locality, Lean-verified): the BDG amplitude
  depends only on the causal past of the proposed vertex. Proved as a
  theorem of discrete DAG dynamics in
  \texttt{RA\_AmpLocality.lean}~\citep{Sandeman2026AmpLoc}, with zero
  sorry tags, by exploiting the transitivity of the causal order.
\item \textbf{L11} ($d=4$ BDG closure, Lean-verified): the BDG action
  in $d=4$ admits exactly five topologically stable classes
  (\texttt{RA\_D1\_Proofs.lean}).
\item \textbf{Benincasa-Dowker uniqueness}~\citep{Benincasa2010}: the
  BDG action is the unique local causal-set action in $d=4$ whose
  continuum limit is the Einstein-Hilbert action
  $\langle\SBDG\rangle \to (l_P^2/4)\,R$.
\end{enumerate}

\noindent The chain proceeds:
\begin{quote}
L01 + O01 + L11 $\;\to\;$ BDG uniqueness (Benincasa-Dowker)
$\;\to\;$ $\langle\SBDG\rangle = (l_P^2/4)\,R$
$\;\to\;$ variation of EH $\;\to\;$ $G_{\mu\nu} = 8\pi G\,\Pact[T_{\mu\nu}]$
$\;\to\;$ LLC on source $\;\to\;$ $\nabla_\mu T^{\mu\nu} = 0$
$\;\to\;$ $\nabla_\mu G^{\mu\nu} = 0$.
\end{quote}

\noindent In standard GR, the geometric Bianchi identity and matter
conservation $\nabla_\mu T^{\mu\nu} = 0$ are mysteriously compatible.
In RA, they are the \emph{same} conservation law---the LLC---at
different levels of description. The Lovelock derivation
(Theorem~\ref{thm:main}) confirms this from the differential geometry
side; the BDG uniqueness derivation confirms it from the discrete side.
Both arrive at the same field equation with $\Lambda = 0$ and no free
parameters.

The RA-native derivation makes explicit that GR is not merely
\emph{recovered} from the discrete structure but is \emph{forced} by it:
the three Lean-verified inputs (L01, O01, L11) leave no room for
alternative field equations. The Lovelock derivation remains valuable as
independent confirmation from established differential geometry.
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Hubble tension}: $H_{\rm eff}(x) \propto 1/\lambda(x)$
    predicts $H_{\rm local} \approx 73.6$~km/s/Mpc from the KBC void
    ($\delta \approx -0.20$), with a falsifiable prediction
    \$H_{\rm Eridanus} \approx 76.8$~km/s/Mpc for the Eridanus
...
\midrule
LLC (Definition~\ref{def:LLC}) & \texttt{RA\_GraphCore.lean} & Lean-proved, 0 sorry \\
Graph Cut Theorem & \texttt{RA\_GraphCore.lean} & Lean-proved, 0 sorry \\
Markov Blanket Shielding & \texttt{RA\_GraphCore.lean} & Lean-proved, 0 sorry \\
Amplitude locality (O01) & \texttt{RA\_AmpLocality.lean} & Lean-proved, 0 sorry \\
Causal invariance (O02) & \texttt{RA\_AmpLocality.lean} & Lean-proved, 0 sorry (unconditional) \\
Koide $K=2/3$ (L09) & \texttt{RA\_Koide.lean} & Lean-proved, 0 sorry \\
BDG closure, 124 cases (L11) & \texttt{RA\_D1\_Proofs.lean} & Lean-proved, 0 sorry \\
$\alpha_{\rm EM}^{-1}=137$ (L08) & \texttt{RA\_D1\_Proofs.lean} & Lean-proved, 0 sorry \\
Frame independence of $\dS$ & \texttt{RA\_AQFT\_Proofs\_v10.lean} & Lean-proved, 0 sorry \\
Rindler stationarity ($\Delta S=0$) & \texttt{RA\_AQFT\_Proofs\_v10.lean} & Lean-proved, 0 sorry \\
...
The combined RA Lean~4 corpus contains \textbf{176 Lean-verified theorems}
across eight files (\texttt{RA\_D1\_Proofs.lean},
\texttt{RA\_AmpLocality.lean}, \texttt{RA\_GraphCore.lean},
\texttt{RA\_Koide.lean}, \texttt{RA\_AQFT\_Proofs\_v10.lean},
\texttt{RA\_Spin2\_Macro.lean}, \texttt{RA\_SteinChen.lean}),
with one intentional sorry (the LQI adapter in
\texttt{RA\_AQFT\_Proofs\_v10.lean}).
All causal graph theorems---including amplitude locality (O01) and
causal invariance (O02)---are fully proved with zero sorry tags.
The type~III$_1$ AQFT framework is the continuum approximation to
these discrete results; in RA's ontology, the finite-dimensional
Lean proofs are fundamental and compatibility with the continuum
is a consistency check, not a foundational requirement.
...
\paragraph{The RA-native derivation and Lovelock's role.}
Section~\ref{sec:ra_native} presents an alternative derivation of the
same field equation that does not invoke Lovelock's theorem at any
stage. The three Lean-verified inputs (L01, O01, L11), combined with
the published Benincasa-Dowker continuum limit, suffice to derive GR
with $\Lambda = 0$ entirely within RA's discrete mathematical
framework. The Lovelock derivation (Theorem~\ref{thm:main}) remains
valuable as independent confirmation from differential geometry, and
demonstrates that the two approaches---one discrete, one
continuous---converge on the same unique field equation. This
convergence is itself a non-trivial consistency check of the framework.
...
A further consequence of the continuum-limit geometry: the Alexandrov
four-volume $(\pi/4!)\,\tau^4$ and the three-sphere volume $(4\pi/3)\,r^3$
produce the geometric factor $G = (4/3)\,d!$, which equals the BDG
confinement parameter $d \times 2^{d-1}$ \emph{only} in $d = 4$.
This identity ensures that the self-consistency equation for hadronic
bound states has a solution, implying that stable composite matter
exists only in four spacetime dimensions
...
\bibitem[Sandeman(2026AmpLoc)]{Sandeman2026AmpLoc}
J.~F. Sandeman.
\newblock {\texttt{RA\_AmpLocality.lean}: Amplitude Locality as a Theorem of BDG Discrete DAG Dynamics}.
\newblock GitHub: \texttt{github.com/jsandeman/Relational-Actualism}, 2026.
