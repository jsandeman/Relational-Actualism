...
of the Wyler formula for $\alpha_{\mathrm{EM}}$.
New result: the per-link actualization probability is identified with the
fine structure constant $p_{\mathrm{th}} = \alpha_{\mathrm{EM}}$ via the
optical theorem, removing $p_{\mathrm{th}}$ as a free parameter.
...
(RATM), extends the BDG particle classification
...
totalling 101 results with zero \texttt{sorry} tags and no new axioms, verify:
...
for all six first-generation fermions; and 70 further results.
\end{abstract}
