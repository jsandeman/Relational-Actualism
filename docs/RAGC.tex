...
that is local and environment-dependent. A bandwidth-partition model
predicts $H_{\mathrm{local}} \approx 73.6\;\mathrm{km\,s^{-1}\,Mpc^{-1}}$
from the KBC void and a new falsifiable consequence: the Eridanus
supervoid ($\delta \approx -0.30$) should give $H \approx
76.8\;\mathrm{km\,s^{-1}\,Mpc^{-1}}$ (derived from BDG integers $+$ $\Omega_b$, zero free parameters). The Jacobson coefficient
...
$v_{\mathrm{flat}}$ and $r_d$. The generative measure (Derivation~1)
is resolved by the O14 uniqueness theorem: the BDG integers are the
unique growth rule with zero free parameters. The macroscopic limit
(Derivation~2) is resolved by the BDG uniqueness chain
(L01+O01+L11 $\to$ Benincasa-Dowker $\to$ Einstein-Hilbert).
The field equation uniqueness (Derivation~4) is
...
uniqueness theorem.
An alternative RA-native derivation via BDG uniqueness
(L01+O01+L11 $\to$ Benincasa-Dowker) arrives at the same result without
Lovelock, using three Lean-verified inputs and one published theorem.
The Einstein field equations are shown to be the unique consistent
macroscopic description of the RA-CSG; and the speed
...
\textperiodcentered{} Cosmological Constant \textperiodcentered{} Dark Matter \textperiodcentered{} Lean~4 Verification \textperiodcentered{} Benincasa-Dowker Action \textperiodcentered{} Poisson-CSG \textperiodcentered{} Eridanus Supervoid \textperiodcentered{} Lovelock Theorem \textperiodcentered{} Causal Bandwidth \textperiodcentered{} BDG Uniqueness
\end{abstract}
\end{titlepage}
...
\paragraph{Note on derivation status.}
The qualitative account above is derivable from the actualization criterion
and the proper-time identification; the precise numerical coefficients in the
curved-spacetime regime require the full covariant framework
(see Open Problem~O2, Section~\ref{sec:openproblems}). What is not
conjectural is the identification of proper time with the actualization count:
this follows directly from the ontological commitment that time is the growing graph.
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection{Universe Nucleation: The Kerr Geometry}
\label{sec:kerr}

At causal severance, a daughter graph nucleates with initial conditions
inherited from the LLC boundary at the severance surface. The
baryon-to-photon ratio $\eta_b$ of the daughter is set by the
Bekenstein-Hawking entropy of the parent:
\begin{equation}
  \eta_b = \frac{N_{\mathrm{baryons}}}{S_{\mathrm{BH}}/k_B}.
  \label{eq:eta_b}
\end{equation}
Our universe has $\eta_b = 6.1 \times 10^{-10}$ with
$N_{\mathrm{baryons}} \approx 10^{80}$, requiring
$S_{\mathrm{BH}}/k_B \approx 10^{90}$. Inverting the
Bekenstein-Hawking formula $S_{\mathrm{BH}} = \pi(M/m_P)^2\,k_B$:
\begin{equation}
  M_{\mathrm{parent}} \approx 2.5\text{--}6.6 \times 10^6\,M_\odot,
  \label{eq:parent_mass}
\end{equation}
a supermassive black hole comparable to Sagittarius~A$^*$
($4.1 \times 10^6\,M_\odot$). Stellar-mass black holes give the
wrong~$\eta_b$; intermediate-mass black holes give the wrong acoustic
scale. The SMBH mass range is uniquely determined.

A real astrophysical SMBH spins, with dimensionless spin parameter
$a_* \in [0,1)$. The Kerr horizon metric function
$\Sigma_+(\theta) = r_+^2 + a^2\cos^2\theta$ is not spherically
symmetric. Decomposing the boundary flux into Legendre polynomials:
\begin{equation}
  \Phi_{\mathrm{boundary}}(\theta) = \Phi_0\bigl[1 +
  \varepsilon_2\,P_2(\cos\theta) +
  \varepsilon_3\,P_3(\cos\theta)\bigr],
  \label{eq:kerr_boundary}
\end{equation}
where $\varepsilon_2(a_*) \approx 0.036\,a_*^2 + 0.326\,a_*^4$
from the Kerr geometry~[CV], and $\varepsilon_3 = 0$
\emph{identically} by equatorial symmetry of the Kerr metric.
Any octupole ($\varepsilon_3 \neq 0$) must come from the accretion
environment: asymmetric infall, jet asymmetry, or tidal distortion.
Both $\varepsilon_2$ and $\varepsilon_3$ share the spin axis---the
quadrupole from geometry, the octupole from the accretion disk.

The daughter universe is born with anisotropic initial conditions:
\begin{equation}
  \frac{\delta\rho}{\rho}(\theta,\phi) =
  \varepsilon_2\,P_2(\cos\theta) +
  \varepsilon_3\,P_3(\cos\theta),
  \label{eq:daughter_ic}
\end{equation}
with the preferred axis equal to the parent's spin axis.

% ─────────────────────────────────────────────────────────────────────────────
\subsection{The $(2/\ell)^2$ Dilution Law and CMB Anomalies}
\label{sec:cmb_anomalies}

After nucleation, the daughter undergoes rapid expansion.
Mode~$\ell$ exits the causal horizon $\ln(\ell/2)$ e-folds
after $\ell = 2$, giving a parameter-free dilution law:
\begin{equation}
  \frac{\text{amplitude at }\ell}{\text{amplitude at }\ell = 2}
  = \left(\frac{2}{\ell}\right)^{\!2}.
  \label{eq:dilution}
\end{equation}
Consequences: $\ell = 3$ retains 44\% amplitude (20\% power);
$\ell = 4$ retains 25\% (6\% power); $\ell \geq 6$ is below 1\%.
For the nucleation signal to be visible at all,
$N_{\mathrm{total}} \approx 64$--$65$ e-folds~[CV].

Planck satellite data reveals five anomalies at the largest CMB
angular scales, each unexplained by $\Lambda$CDM:
(1)~$C_2 \approx 200\,\mu\mathrm{K}^2$ vs.\
$\Lambda\mathrm{CDM} \approx 1100\,\mu\mathrm{K}^2$ (80\%
suppression); (2)~$\ell = 2$ and $\ell = 3$ axes aligned to
$9$--$13^\circ$; (3)~N/S hemisphere asymmetry
$A_{\mathrm{NS}} \approx 0.07$; (4)~$C_3$ consistent with
$\Lambda$CDM; (5)~$\ell \geq 4$ normal.

All five follow from the Kerr nucleation geometry:
\begin{enumerate}[nosep]
\item The coherent nucleation quadrupole partially cancels the
  stochastic (inflationary) quadrupole, suppressing~$C_2$.
\item Both $\varepsilon_2$ and $\varepsilon_3$ are aligned with the
  parent's spin axis. The alignment is structural, not coincidental.
\item The $\varepsilon_3 P_3(\cos\theta)$ term is odd under
  $\theta \to \pi - \theta$, creating hemisphere asymmetry aligned
  with the spin axis.
\item The $(2/\ell)^2$ dilution makes the $\ell = 3$ nucleation
  signal 44\% of $\ell = 2$---too weak to suppress~$C_3$ but
  sufficient for alignment.
\item The $(2/\ell)^2$ dilution makes the nucleation signal
  negligible at $\ell \geq 4$.
\end{enumerate}

The model has three parameters: parent spin~$a_*$, extra
e-folds~$\Delta N$ beyond minimum inflation, and accretion
asymmetry $\delta_{\mathrm{acc}} =
\varepsilon_3/\varepsilon_2 \approx 0.69$. These are constrained
by three observations ($C_2$ suppression, alignment angle, N/S
asymmetry) with two consistency checks ($C_3$ not suppressed,
higher~$\ell$ normal). The system is just-determined: \emph{zero
remaining freedom}. For any $a_* \in [0.3, 0.998]$,
$\Delta N \approx 3.3$--$5.0$, giving
$N_{\mathrm{total}} \approx 64$--$65$~[CV].

$\Lambda$CDM has no mechanism for any of these anomalies. RA
predicts all five as structural consequences of Kerr nucleation.

% ─────────────────────────────────────────────────────────────────────────────
\subsection{The Cosmic Web as Dynamical Attractor}
\label{sec:cosmic_web}

The nucleation perturbations seed structure formation through a
feedback mechanism intrinsic to RA. The field equations couple local
expansion rate inversely to baryon density: underdense regions expand
faster, further diluting; overdense regions expand slower, further
concentrating. This positive feedback drives density perturbations
away from uniformity.

The result is the filamentary cosmic web. In $\Lambda$CDM, this
structure is contingent on the initial perturbation spectrum. In RA,
the feedback makes filamentary topology \emph{inevitable} for any
universe with density perturbations. The cosmic web is the dynamical
attractor of any RA universe, regardless of specific initial conditions.

The large-scale orientation of the cosmic web should correlate with the
CMB preferred axis (the parent's spin axis). This is testable with
DESI/Euclid galaxy surveys. $\Lambda$CDM predicts no such correlation.

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Dual Severance and the Fate of the Universe}
\label{sec:fate}

The void-filament feedback does not stop. Both endpoints of the
density bifurcation lead to causal disconnection:

\paragraph{Starvation severance.}
Voids approach the Erd\H{o}s-R\'enyi percolation threshold $\mu = 1$,
below which the causal graph fragments. The graph stops growing
locally---quiet disconnection by depletion.

\paragraph{Density severance.}
Dense filament boundaries approach $\mu_2 \approx 5.7$, where
$P_{\mathrm{acc}} \to 0$ and new severances fire, nucleating daughter
universes.
...
fragmentation prevents this: every density perturbation drives
regions away from $\mu = 1$, and both directions terminate in
disconnection. The universe does not equilibrate. It \emph{fragments}.

At dense boundaries, the cycle begins again. Each daughter inherits
initial conditions from its parent's local structure, including
angular momentum. The physics is identical across
generations---only initial conditions vary. The ``multiverse'' of RA
is a multiverse of initial conditions, not of different physics.

% ─────────────────────────────────────────────────────────────────────────────
\subsection{Boltzmann Brain Prohibition}
\label{sec:boltzmann}

In $\Lambda$CDM with $\Lambda > 0$ and infinite time, Boltzmann
Brain nucleations are inevitable and should vastly outnumber ordinary
observers.

RA prohibits Boltzmann Brains by two mechanisms. First,
\emph{structurally}: the actualization filter selects based on
relative entropy, not Boltzmann statistics; random fluctuations
cannot pass the filter to build ordered complexity because each of
the $\sim\!10^{25}$ required actualization steps must independently
cross the $\Delta S^*$ threshold. Second, \emph{dynamically}: no
causal patch persists long enough for BB nucleation because
fragmentation (Section~\ref{sec:fate}) disconnects every component
on a finite timescale. The combination is a categorical prohibition,
not merely a probabilistic suppression.
...
\paragraph{RA-native derivation of the bandwidth ratio.}
The ratio $\bar\lambda_m/\bar\lambda_{\mathrm{exp}}$ is not a free parameter.
At cosmological scales, the matter fraction of the total actualization bandwidth
equals the effective matter density as a fraction of critical density:
\begin{equation}
  \frac{\bar\lambda_m}{\bar\lambda_{\mathrm{exp}}}
  = \frac{\Omega_m^{\mathrm{RA}}}{1 - \Omega_m^{\mathrm{RA}}},
  \qquad
  \Omega_m^{\mathrm{RA}} = \Omega_b\,(1 + f_0),
  \label{eq:lambda_ratio_derived}
\end{equation}
where $f_0 = 5.42$ is the BDG bandwidth enhancement factor
(derived from BDG integers with zero free parameters;
see RASM~\citep{Sandeman2026RASM}) and $\Omega_b = 0.0493$
(Planck 2018) is the baryon fraction---an initial condition set at
the Big Bang causal severance, not a parameter of the theory.
This gives:
\begin{equation}
  \Omega_m^{\mathrm{RA}} = 0.0493 \times 6.42 = 0.317,
  \qquad
  \frac{\bar\lambda_m}{\bar\lambda_{\mathrm{exp}}} = \frac{0.317}{0.683} = 0.463.
\end{equation}

\paragraph{Consistency check: the KBC void.}
...
value $\bar H = 67.4\;\mathrm{km\,s^{-1}\,Mpc^{-1}}$, the KBC-calibrated
ratio would be $0.084/0.20 \approx 0.42$.  The derived value $0.463$
reproduces $H_{\mathrm{local}} = 73.0$ at $|\delta| = 0.179$---well within the
${\pm}15$--$20\%$ observational uncertainty on $\delta_{\mathrm{KBC}}$.
Equivalently, at the nominal $|\delta| = 0.20$, the derived ratio predicts
$H_{\mathrm{local}} \approx 73.6\;\mathrm{km\,s^{-1}\,Mpc^{-1}}$,
consistent with SH0ES within $1\sigma$.
...
equation~\eqref{eq:hubble_partition_full} with the derived ratio:
\begin{equation}
  H_{\mathrm{Eridanus}}
  = 67.4 \times \bigl(1 - (-0.30)(0.463)\bigr)
  \approx 76.8\;\mathrm{km\,s^{-1}\,Mpc^{-1}}.
...
direction of the Eridanus supervoid consistent with ${\sim}76.8\;\mathrm{km\,s^{-1}\,Mpc^{-1}}$
...
\subsection{Prediction 5: CMB Anomalies from Kerr Nucleation}
\label{sec:pred_cmb}

The Kerr nucleation model (Section~\ref{sec:kerr}) predicts:

\begin{enumerate}[nosep,label=(\alph*)]
\item The CMB quadrupole-octupole alignment (``axis of evil'')
  persists in CMB-S4 data at ${>}\,3\sigma$ significance, because it
  is a structural consequence of the parent SMBH spin axis, not a
  statistical fluctuation.
\item The N/S hemisphere asymmetry is aligned with the axis-of-evil
  axis, because both originate from the parent's equatorial plane.
\item Higher CMB multipoles ($\ell \geq 4$) show decreasing alignment
  with the preferred axis, following the $(2/\ell)^2$ dilution
  law~\eqref{eq:dilution}.
\item The parent SMBH mass is $M_{\mathrm{parent}} \approx
  2.5$--$6.6 \times 10^6\,M_\odot$~\eqref{eq:parent_mass}.
\end{enumerate}

\noindent
$\Lambda$CDM has no mechanism for any of these. A detection of
the $(2/\ell)^2$ scaling in CMB-S4 data would constitute strong
evidence for the nucleation cosmology.

\subsection{Prediction 6: Near-Minimum Inflation}
\label{sec:pred_inflation}

For the nucleation signal to be visible at all, the universe must have
undergone nearly the minimum amount of inflation:
$N_{\mathrm{total}} \approx 64$--$65$ e-folds~[CV]. This is itself a
prediction, testable via the tensor-to-scalar ratio~$r$: near-minimum
inflation predicts a specific range of~$r$ dependent on the
inflationary potential.

\subsection{Prediction 7: Cosmic Web Alignment with CMB Axis}
\label{sec:pred_web}

The large-scale orientation of the cosmic web should correlate with the
CMB preferred axis (Section~\ref{sec:cosmic_web}). Testable with
DESI/Euclid galaxy surveys. $\Lambda$CDM predicts no such
correlation, making this a clean discriminant.

\subsection{Prediction 8: No Heat Death --- Fragmentation}
\label{sec:pred_fate}

The dual severance mechanism (Section~\ref{sec:fate}) predicts that
the universe fragments rather than equilibrates. Every causally
connected component has a finite lifetime, bounded by the timescale
on which void starvation or boundary nucleation disconnects it.
Boltzmann Brains are categorically prohibited
(Section~\ref{sec:boltzmann}), not merely suppressed.

\section{Mathematical Derivations}
\label{sec:math}

The following derivations represent the core mathematical tasks of
RAGC. Two of these (Problems~1 and~2) have been resolved since
initial drafting: Problem~1 by the O14 uniqueness theorem (the BDG
integers are the unique growth rule, with zero free parameters) and
Problem~2 by the BDG uniqueness chain (L01+O01+L11 $\to$
Benincasa-Dowker $\to$ Einstein-Hilbert). The derivation content
below is retained as it provides the detailed proof architecture.
The predictions of Section~\ref{sec:predictions} stand on the basis of the
postulates and proved theorems.
...
\subsubsection{Derivation Status: Generative Measure}

\textbf{The primary open problem (continuum limit) is now closed.}
...
$\Lambda = 0$ and no free parameters. RACL additionally presents an
RA-native derivation via BDG uniqueness (L01+O01+L11 $\to$ Benincasa-Dowker)
that does not require Lovelock's theorem. The BDG locality lemma in RACL
...
The coupling constants $c_k$ in the RA-CSG transition probabilities ---
which in standard Rideout-Sorkin CSG are genuinely free
parameters~\citep{Sorkin2003} --- are uniquely fixed in RA by the O14
uniqueness theorem: binomial inversion of the $d=4$ chord diagram
moments (Yeats 2025) yields $(1,-1,9,-16,8)$ with zero free parameters.
...
The type~III$_1$ von Neumann algebra framework (Araki's modular relative
entropy~\citep{Araki1976}, Tomita-Takesaki modular flow) is the continuum
approximation to these discrete results. In RA's ontology, the
finite-dimensional Lean proofs are fundamental---the discrete causal graph
is the primitive reality. The type~III$_1$ algebra is what the discrete
structure looks like when coarse-grained into continuum AQFT at
$\mu \gg 1$. Showing compatibility between the discrete results and
the continuum framework is a consistency check with the same epistemic
status as the dissolved problems O10 (discrete Bianchi $=$ LLC in
continuum costume) and O11 (Lorentz invariance $=$ causal invariance in
continuum costume). It is not a foundational requirement.
...
This does not imply that accelerated observers cannot experience physical heating:
if an accelerated body is in contact with a physical heat bath
(friction, collisions, electromagnetic radiation from external sources),
these involve genuine on-shell exchanges with the environment and do produce
$\Delta S > 0$ actualization events. The Unruh thermal bath is
specifically the \emph{stationary} KMS state of the Minkowski vacuum
as seen by the accelerated observer --- it does not thermalize a body further
than the vacuum, because $\Delta S_{\mathrm{rel}} = 0$ under modular flow.
...
These finite-dimensional results are the fundamental physics in RA's
discrete ontology. The type~III$_1$ continuum AQFT framework is the
macroscopic approximation; compatibility is a consistency check, not a
prerequisite for the discrete results.
...
\subsubsection{Derivation Status and Open Problems}
\label{sec:hardwall}
...
\subsection{Derivation 5: The Unified Metric Sourcing Equation}
\label{sec:d5}

\paragraph{Status: relocated to RADM.}
The conjectural two-component metric sourcing equation
$\nabla^2\Phi = 4\pi G[\rho_A + \rho_\lambda]$, the derivation of the
$\nabla^2(\ln\lambda)$ gradient term, and the associated rotation curve and Bullet
Cluster arguments are developed as the primary subject of the companion paper
RADM~\citep{Sandeman2026RADM}. That paper provides the detailed derivation
candidates, hard walls, and collaborator targets for the dark matter replacement
programme. The WIMP prohibition (Prediction~4 above) stands independently of
the conjectural mechanisms in RADM.
...
\paragraph{Bounding estimate for WEP correction.}
While the formal proof is a hard wall, the order of the correction
can be estimated. For a species in actualization equilibrium, the
deviation from WEP scales as:
\begin{equation}
  \frac{|\rho_A - \rho_{\mathrm{matter}}|}{\rho_{\mathrm{matter}}}
  \;\sim\; \frac{\tau_{\mathrm{eq}}}{t_{\mathrm{cosm}}},
  \label{eq:wep_bound}
\end{equation}
where $\tau_{\mathrm{eq}}$ is the timescale for the species to reach
actualization equilibrium and $t_{\mathrm{cosm}}$ is the age of the
universe. For ordinary baryonic matter, which has been in thermal and
electromagnetic equilibrium since recombination:
$\tau_{\mathrm{eq}} \sim 10^{13}$~s (recombination epoch),
$t_{\mathrm{cosm}} \sim 4 \times 10^{17}$~s, giving a fractional
deviation $\lesssim 10^{-4}$, well below current E\"{o}tv\"{o}s
precision ($\sim 10^{-13}$) in the post-recombination regime.
The deviation is negligible not because WEP is assumed, but because
ordinary matter has had sufficient time to reach actualization
equilibrium. WIMPs never reached this equilibrium.

\paragraph{Hard wall: formal proof of WEP recovery.}
The argument above establishes the equilibrium limit qualitatively;
...
\begin{itemize}[noitemsep]
  ...
  measurements. This is the \emph{theoretically preferred} proxy: since RA
  privileges on-shell electromagnetic and strong interactions
  ($\Delta S_{\mathrm{rel}} > 0$ from EM/strong on-shell processes), $\rho_b$
  is the most direct observable correlate of accumulated actualization flux.
  Galaxy number density and star-formation rate are secondary proxies
  (correlated with $\rho_b$ but with higher systematic uncertainty);
  \textbf{the primary falsifiable prediction uses $\rho_b$}.
  \label{sec:proxies}
\end{itemize}
...
\paragraph{Upstream dependency: the generative measure $c_k$ --- resolved.}
The coupling constants $c_k$ are uniquely fixed by the O14 uniqueness
theorem. The quantitative $H_0$ forecast requires computing the
bandwidth-partition ratio from the unique BDG growth rule at $\mu = 1$
--- a specific numerical extraction, no longer a conceptual open problem.
The bandwidth-partition formula
and Eridanus prediction are stated as Prediction~1b
(Section~\ref{sec:hubble_partition}). The ratio
$\bar\lambda_m/\bar\lambda_{\mathrm{exp}} = 0.463$ is derived
from $\Omega_b(1+f_0)/(1 - \Omega_b(1+f_0))$ with $f_0 = 5.42$ (BDG).
\textbf{Collaborator target:} observational cosmology, large-scale structure.

\subsection{Summary: Status of Each Derivation}

\begin{description}
  \item[Derivation 1 (Generative Algebra).] \textit{Resolved.} The
  O14 uniqueness theorem establishes that the BDG integers
  $(1,-1,9,-16,8)$ are the unique growth rule for a $d=4$ causal
  graph, via M\"obius inversion of the chord diagram moment sequence
  (Yeats 2025, Rota 1964). The coupling constants follow with zero
  free parameters: $\alpha_{\mathrm{EM}}^{-1} = 137.036$ (IC30,
  discrete Dyson equation, 0.00001\% of PDG);
  $\alpha_s(m_Z) = 1/\sqrt{72} = 0.11785$ (0.13\% of PDG). The
  Poisson-CSG formula
  $c_k^{\mathrm{RA}}(x) = e^{-\mu}\mu^k/k!$ with $\mu = \lambda V_{\mathrm{coh}}
  p_{\mathrm{th}}$ remains the local, environment-dependent dynamics;
  the BDG sign criterion (equation~\eqref{eq:BDG_filter}) is the
  discrete RA actualization filter.
  \textbf{Status: resolved by O14 uniqueness.}

  \item[Derivation 2 (Macroscopic Limit and GR Inevitability).]
  \textit{Resolved.} This derivation decomposes
  ...
  An alternative RA-native derivation does not require Lovelock: three
  Lean-verified results (L01, O01, L11) combined with the published
  Benincasa-Dowker continuum limit theorem yield the Einstein-Hilbert action
  directly, with the Bianchi identity as the LLC at the metric level (see
  RACL~\citep{Sandeman2026RACL}, Section~5.1).
  ...
  gravitational-wave dispersion. \textbf{Status: resolved by both Lovelock
  and BDG uniqueness chains. Planck-scale corrections: open.}

  \textbf{Former gap (D2c$'$) --- dissolved:} The type~III$_1$ AQFT
  extension (Araki's modular relative entropy, Tomita-Takesaki modular
  flow) is the continuum approximation to the discrete Lean-verified
  results. In RA's ontology, the finite-dimensional proofs are
  fundamental; the continuum framework is what they look like when
  coarse-grained. This gap has the same dissolved status as O10
  (Bianchi = LLC in continuum costume) and O11 (Lorentz = causal
  invariance in continuum costume). Compatibility is expected as a
  consistency check but is not load-bearing for the framework.
...
decoherence limits.
A specific numerical estimate is now available from the companion paper
RAQM~\citep{Sandeman2026RAQM}: the per-vertex entropy generated at
actualization threshold is $\langle\Delta S_{\mathrm{vertex}}\rangle
= -\log P_{\mathrm{acc}}(1) \approx 0.601$~nats (derived from the
BDG Poisson-CSG at $\mu=1$). Substituting into~\eqref{eq:xi_microscopic}
with the Planck area $l_P^2$:
\begin{equation}
  \xi \;\approx\; \frac{0.601~\text{nats}}{l_P^2}
  \;\approx\; 0.601\,l_P^{-2},
  \label{eq:xi_numerical}
\end{equation}
a parameter-free prediction connecting the quantum measurement threshold
to the coupling constant governing galactic rotation curves.
This estimate is provisional pending the full derivation of the RA
continuum limit, but it is no longer undetermined in principle.
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Open Problems and Future Horizons}
\label{sec:openproblems}
% ─────────────────────────────────────────────────────────────────────────────

The following problems are precisely scoped open targets for the RA programme.
Completed results are not repeated here; this section gathers only the
outstanding mathematical derivations and their collaboration requirements.

\begin{description}[leftmargin=2em,labelwidth=!,labelsep=0.5em]

\item[\textbf{O1: Covariant generalisation of the generative measure.}]
The BDG-filtered Poisson-CSG defines the actualization dynamics in the
flat-spacetime, weak-field regime. Extension to general curved backgrounds
requires deriving the covariant generative measure for the RA-CSG, confirming
that the BDG filter is invariant under diffeomorphisms of the underlying
continuum approximation.
\textit{Collaboration:} causal set theory; CDT computational physics;
causal dynamical triangulations.

\item[\textbf{O2: Covariant Step~4 (stochastic update commutativity).}]
The stochastic graph update in the Engine of Becoming must be shown to
commute for spacelike-separated candidates: selecting $v_A$ then $v_B$
must produce the same graph as $v_B$ then $v_A$ when causally disconnected.
This is the Tomonaga-Schwinger analogue for the stochastic update.
\textit{Collaboration:} algebraic QFT; causal set theory.

\item[\textbf{O3: Non-perturbative regime of $\mathcal{P}_{\mathrm{act}}$.}]
The actualization projector uses Fock-space structure, requiring a perturbative
vacuum. In strongly coupled regimes (QCD confinement, critical points), a
non-perturbative formulation via Haag-Kastler axioms or the OPE is required.
Extension to curved backgrounds via Hadamard renormalization is also needed.
\textit{Collaboration:} algebraic QFT; Tomita-Takesaki modular theory.

\item[\textbf{O4: Formal WEP recovery.}]
The bounding estimate $|\rho_A - \rho_{\mathrm{matter}}|/\rho_{\mathrm{matter}}
\lesssim \tau_{\mathrm{eq}}/t_{\mathrm{cosm}}$ (Section~\ref{sec:wep_bound})
establishes WEP consistency phenomenologically.
A formal proof that $\rho_A \to \rho_{\mathrm{matter}}$ in the equilibrium limit,
with explicit bounds as a function of departure from equilibrium, would
complete the WEP recovery argument and fully justify the WIMP prohibition.
\textit{Collaboration:} experimental gravity; precision equivalence principle tests.

\item[\textbf{O5: Fixing $\xi$ from RA microscopic structure.}]
The coupling constant $\xi \approx 0.601\,l_P^{-2}$ (Section~\ref{sec:xi})
is estimated from the RAQM actualization threshold. A first-principles
derivation from the entropy generated per actualization event and the
RA area element would promote this from an estimate to a prediction.
The RAQI Kinematic Coherence Bound provides a second handle.
\textit{Collaboration:} quantum information; quantum gravity phenomenology;
large-scale structure.

\item[\textbf{O6: Quantitative $H_0$-density slope.}]
Prediction~1 (Eridanus supervoid $H_{\mathrm{Eridanus}} \approx 76.8$~km/s/Mpc)
uses the baryon density $\rho_b$ as the theoretically favoured proxy for
$\lambda(x)$ (see Section~\ref{sec:proxies}). A quantitative derivation
of the $H_0$-density slope from $\xi$ and $\bar\rho_A$ would make this
prediction fully parameter-free.
\textit{Collaboration:} observational cosmology; DESI; large-scale structure surveys.
...
\begin{description}[leftmargin=1.5em]
  ...
  on causal edges that distinguishes left-handed from right-handed motifs. This
  is now resolved: DAG acyclicity forbids backward-pointing edges, and the BDG
  filter rejects the backward-winding pattern $N=(1,1,1,0)$ (score $= -7 < 0$)
  while preserving the forward-winding $N=(1,1,0,1)$ (score $= +17 > 0$).
  Parity violation is exactly maximal---a topological impossibility, not a
  statistical preference. The proof is Lean-verified
  (\texttt{RA\_BaryonChirality.lean}, zero sorry tags).
...
\paragraph{New: the mass hierarchy from BDG.}
A multiplicative cascade through BDG depth layers gives the proton mass
$m_p = m_P\,\alpha_{\mathrm{EM}}^5/2^{28} = 941~\mathrm{MeV}$
($0.3\%$ error) and the Higgs mass
$m_H = (\alpha_{\mathrm{EM}}^{-1} - L_q)\,m_p = 125.2~\mathrm{GeV}$
($0.06\%$), from BDG integers and the Planck mass alone. The $d = 4$
geometric identity $\frac{4}{3}\,d! = d \times 2^{d-1}$ ensures that the
self-consistency condition for hadronic bound states has a solution
exclusively in four spacetime dimensions. Full derivation:
RASM~\citep{Sandeman2026RASM}.
...
\newblock \href{https://doi.org/10.5281/zenodo.19270020}{doi:10.5281/zenodo.19270020}.
