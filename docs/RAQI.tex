...
can occur. Unlimited scaling without improving $\eta$ will eventually push the
logical error rate above $p_{\mathrm{th}}$ regardless of single-qubit fidelity.
...
A key technical result: the quantum relative entropy is frame-independent
(\texttt{frame\_independence} in \texttt{RA\_AQFT\_Proofs\_v10.lean},
zero sorry tags, Lean~4.29.0), proved using the continuous functional
calculus unitary-conjugation lemma. The Rindler thermal state
\texttt{rindlerThermal} and \texttt{rindler\_stationarity} are also
fully proved with zero sorry tags in the same file. The
...
Throughout, implications are stated conditionally on the correctness of the RA
framework, which is currently under peer review.
\end{abstract}
...
events. This is the theorem \texttt{rindler\_stationarity}, now
fully proved in Lean~4.29.0 with zero sorry tags; the supporting
density matrix \texttt{rindlerThermal} (hermiticity, positive
semidefiniteness, and trace~1) is also fully proved.
...
Poincar\'{e}-invariance of the vacuum (a QFT physics input), and the data processing inequality (proved in Lean-QuantumInfo and
also established natively in \texttt{RA\_AQFT\_Proofs\_v10.lean}
via \texttt{CPTPMap.apply}). The extension to the full
