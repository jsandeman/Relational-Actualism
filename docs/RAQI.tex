...
condition $p^\mu p_\mu = m^2c^2$, at which a mediating boson crosses the
...
on decoherence and yields the \emph{Kinematic Coherence Bound}: the maximum
fault-tolerant array size is $N_{\max} = \eta \cdot p_{\mathrm{th}}$,
where $\eta$ is the single-qubit quality factor and $p_{\mathrm{th}}$ is the
...
machine-verified Lean~4 theorem (\texttt{frame\_independence} in
\texttt{RA\_AQFT\_Proofs\_v10.lean}), proved using the continuous functional
calculus unitary-conjugation lemma of the Lean-QuantumInfo library. The
fault-tolerance threshold, the speed of light $c = l_P/t_P$, and the biological
Causal Firewall are all the same Erd\H{o}s-R\'{e}nyi percolation transition
...
\paragraph{Decoherence vs.\ actualization: why QEC cannot unwind a Kinematic Snap.}
It is essential to distinguish two physically distinct processes that both
cause quantum information loss.

\emph{Decoherence} is unitary entanglement of the qubit with its environment.
It is reversible in principle: the qubit-environment system evolves under a
unitary that, if reversed, would recover the qubit state. Standard quantum
error correction (QEC) --- surface codes, stabiliser codes, bosonic codes ---
is designed precisely to undo this unitary entanglement before it propagates
beyond the correction radius. QEC works because decoherence is a \emph{reversible
unitary process}.

\emph{Actualization} (the Kinematic Snap) is an ontological, non-unitary
graph-rewriting event. When the qubit's interaction with its environment
satisfies $\Delta S(\rho \| \sigma_0) > 0$, a permanent directed edge is
inscribed into the causal DAG. This is not a unitary rotation: it is an
irreversible change in the causal structure of spacetime. The superposition is
destroyed at the fundamental graph level \emph{before} any syndrome measurement
can be executed --- the collapse is not a change of the wavefunction on an
existing graph; it is a rewriting of the graph itself.

QEC operates on the wavefunction. It cannot undo a graph rewrite.

The practical consequence is that there exist errors that are in principle
uncorrectable: those caused by genuine Kinematic Snaps rather than by
decoherence. Any error correction protocol that identifies a Snap as if it were
decoherence will fail because the relevant degrees of freedom are no longer in
superposition --- they have been permanently inscribed into the DAG. The
$N_{\max}$ bound of the KCB (Section~\ref{sec:kcb}) is precisely the bound
on the number of qubits before Snap-type errors overwhelm the correctable
decoherence budget.
...
\paragraph{Connection to Standard Model coupling constants.}
The actualization threshold $p_{\mathrm{th}} \approx 10^{-2}$ used in the
Kinematic Coherence Bound is distinct from, but related to, the fundamental
coupling constants of the Standard Model. In the companion paper
RASM~\citep{Sandeman2026RASM} and the focused derivation
paper~\citep{Sandeman2026BDG}, the strong coupling constant is derived directly
from the BDG integers: $\alpha_s(m_Z) = 1/\sqrt{c_2 c_4} = 1/\sqrt{72} = 0.11785$,
agreeing with the PDG world average to $0.13\%$.  The coupling $\alpha_s$ governs
actualization events in the strong sector; $p_{\mathrm{th}}$ governs the
\emph{error-correction threshold} in engineered quantum arrays.  These operate at
different scales and serve different roles in the RA framework.  Their numerical
proximity ($p_{\mathrm{th}} \sim 10^{-2}$, $\alpha_s \sim 10^{-1}$) reflects the
common origin in BDG causal geometry but is not an identity.
...
processing inequality (proved in Lean-QuantumInfo). In RA's ontology,
these finite-dimensional results are fundamental---the discrete causal
graph is the primitive reality. The type~III$_1$ continuum AQFT framework
is the macroscopic approximation; compatibility is a consistency check,
...
compiled theorem. The type~III$_1$ von Neumann algebra framework is the
continuum approximation to these discrete results; RA treats the
finite-dimensional proofs as fundamental.
...
\textbf{Code availability:} The Lean~4 formal proof files (\texttt{RA\_D1\_Proofs.lean}, \texttt{RA\_AmpLocality.lean}, \texttt{RA\_GraphCore.lean}, \texttt{RA\_Koide.lean}, \texttt{RA\_AQFT\_Proofs\_v10.lean}, \texttt{RA\_Spin2\_Macro.lean}, \texttt{RA\_SteinChen.lean}) and Python computational scripts are publicly available at \href{https://github.com/jsandeman/Relational-Actualism}{github.com/jsandeman/Relational-Actualism}. The RA Lean~4 corpus comprises 176 verified theorems with one intentional sorry (LQI adapter). Amplitude locality (O01) and causal invariance (O02) are proved unconditionally in \texttt{RA\_AmpLocality.lean}.
