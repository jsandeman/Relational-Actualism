...
\texttt{RA\_D1\_Proofs.lean} contains the particle classification core with zero \texttt{sorry} tags and no
axioms beyond the Mathlib standard library. The full RA Lean~4 corpus now comprises \textbf{176 Lean-verified theorems} across eight files---including amplitude locality (O01, proved as a theorem of discrete DAG dynamics in \texttt{RA\_AmpLocality.lean}), causal invariance (O02, unconditional), the Local Ledger Condition and graph cut theorem (\texttt{RA\_GraphCore.lean}), and the Koide formula (\texttt{RA\_Koide.lean})---with one intentional sorry (the LQI adapter~\citep{Meiburg2025LQI} in \texttt{RA\_AQFT\_Proofs\_v10.lean}). This constitutes the first machine-verified particle classification in the
...
\paragraph{Concrete quantum-number example: three colour charges from $N_1 = 2$.}
Type-3 vertices have $\mathbf{N} = (2,1,0,0)$ and BDG score
$S_{\mathrm{BDG}} = c_4 = 8$.
The $N_1 = 2$ value is the unique distinguishing feature of this class:
no other stable topology class in $d=4$ has $N_1 > 1$.
In the BDG-filtered Poisson-CSG, the $N_1$-fold depth-1 degeneracy
admits exactly $N_1 + 1 = 3$ distinct causal configurations
at depth~1 — corresponding to the three colour charges of QCD (red,
green, blue).
The Lean-verified confinement length $L_q = 4$
(Theorem~\ref{thm:D1e}) establishes directly that Type-3 vertices
cannot propagate as free asymptotic states beyond causal depth~4,
reproducing colour confinement as a theorem of the BDG integer
structure, not as an external input.
This provides a concrete checkable example of the bijection between
BDG topology classes and SM quantum numbers: the $N_1 = 2$
degeneracy encodes $\mathrm{SU}(3)_{\mathrm{colour}}$
multiplicity, and the Lean-proved $L_q = 4$ encodes confinement.

\begin{remark}[Physical confirmation of $L_q = 4$]
The confinement length $L_q = 4$ predicts the proton charge radius:
$r_p = L_q \times \hbar/(m_p c) = 4 \times 0.210~\mathrm{fm} = 0.84~\mathrm{fm}$,
matching the CODATA value $0.8414 \pm 0.0019~\mathrm{fm}$ to $0.03\%$.
Moreover, the $d = 4$ geometric identity $\frac{4}{3}\,d! = d \times 2^{d-1}$
(holding only in $d = 4$) is what allows the self-consistent cascade mass
formula $m_p = m_P\,\alpha_{\mathrm{EM}}^5/2^{28} = 941~\mathrm{MeV}$ ($0.3\%$).
Stable hadronic bound states exist only in four spacetime dimensions
...
\subsection{Gauge group structure from BDG signs (GS02)}
\label{sec:gs02}

The Standard Model gauge group $\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)$ emerges from the alternating sign structure of the BDG coefficients $(+,-,+,-,+)$, which is forced by the inclusion--exclusion counting of causal intervals in $d=4$. At each depth~$k$, the sign determines whether the corresponding layer contributes isotropically ($c_k > 0$, encouraging many ancestors distributed across spatial directions) or anisotropically ($c_k < 0$, penalising multiple ancestors and selecting a preferred direction):
...
\midrule
0 & $+1$  & $+$ & baseline (vacuum) & --- \\
1 & $-1$  & $-$ & $E[N_1]=1$: one ancestor $\to$ 2+1 split & $\mathrm{SU}(2)_L \times \mathrm{U}(1)_Y$ \\
2 & $+9$  & $+$ & distributed $\to$ scalar & $\mathrm{U}(1)$ remnant (Higgs) \\
3 & $-16$ & $-$ & heavily penalised $\to$ confined & inherits $\mathrm{SU}(3)$ \\
4 & $+8$  & $+$ & distributed $\to$ isotropic & native $\mathrm{SU}(3)_c$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent The 2+1 split at depth~1 has a concrete origin: at $\mu=1$, $\lambda_1 = 1$, so each vertex has on average exactly one nearest causal ancestor. One ancestor picks one spatial direction, breaking three-dimensional isotropy into two transverse directions ($\mathrm{SU}(2)_L$) and one longitudinal ($\mathrm{U}(1)_Y$). The product $\mathrm{SU}(3) \times \mathrm{SU}(2) \times \mathrm{U}(1)$ is confirmed by exact enumeration: $E[N_k \mid S > 0]$ is strictly greater than $E[N_k]$ for positive~$c_k$ and strictly less for negative~$c_k$, for all $k = 1,\ldots,4$~[CV].

This result connects the particle classification of the present paper to the gauge structure of the Standard Model: the BDG integers not only determine which topology classes are stable (Sections~3--6) but also determine which gauge symmetries govern the interactions between those classes. The gauge group is not imposed on the framework; it is a counting consequence of the same five integers $(1,-1,9,-16,8)$ that determine the particle spectrum.

% ── 8. Coherent State and Isospin ───────────────────────────────────────────
\section{The Coherent State Theorem and Isospin Structure}
\label{sec:D1h}

The Lean-proved theorem \texttt{koide\_formula}
(in \texttt{RA\_Koide.lean}) establishes:
\[
...
in the causal set theory literature. The RA Lean~4 corpus now comprises
176~theorems across eight files with one intentional sorry (the LQI adapter).
Since initial submission, amplitude locality (O01) has been proved as a theorem
of discrete DAG dynamics (\texttt{RA\_AmpLocality.lean}, zero sorry tags),
closing the causal invariance theorem unconditionally.
