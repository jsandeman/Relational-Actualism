...
\begin{abstract}
Relational Actualism (RA) is a discrete framework for fundamental physics developed in the immediate lineage of causal set theory, but with a stronger ontological and dynamical commitment. Like causal set theory, it treats physical reality as a locally finite causal order and uses sequential growth and the Benincasa--Dowker--Glaser (BDG) action as native discrete structure. Unlike standard presentations of causal set theory, however, RA makes irreversible actualization primitive, treats the realized graph and its structured potentia as the full physical state, imposes the local ledger condition at every vertex, and uses the $d=4$ BDG score itself as the local acceptance kernel for event formation. The purpose of this paper is to establish that kernel coherently and explicitly: the seven axioms, the finitary growth rule, the graph-cut / ledger structure of realized partitions, the criterion $S_{\mathrm{BDG}}>0$, the kernel-locality family showing that admissibility depends only on realized past, and the arithmetic coefficient-inversion layer supporting the $d=4$ kernel. Throughout, the paper compares RA with causal set theory, quantum mechanics, and general relativity without treating any of those frameworks as RA's proof target. Where RA already offers a structurally different explanation---for example, by making actualization primitive rather than adding a Born rule or collapse postulate---that difference is stated directly. Where RA does not yet have theorem-level closure---for example, in deriving quantum-statistical frequency laws, in closing the macroscopic kinematic programme, or in giving a fully native geometric proof of the $d=4$ coefficient tuple---the gap is stated explicitly together with a native route forward. The active compiled support surface used positively in this paper is deliberately narrow: \texttt{RA\_GraphCore.lean}, \texttt{RA\_O01\_KernelLocality\_v2.lean}, and \texttt{RA\_O14\_ArithmeticCore\_v1.lean}.
\end{abstract}
...
The present paper is therefore best read as the kernel manifesto and kernel audit of the RA suite. It argues that the primitive physical fact is the writing of a vertex into a growing causal DAG, and that a surprising amount of structure follows already from that assumption once it is combined with the BDG filter and the LLC. Everything downstream in Papers~II--IV depends on whether this kernel is coherent.
...
Only $d=4$ places the first selectivity ceiling close to the kernel scale $\mu=1$ while also supplying the finite motif richness used in the Paper~II census. This is therefore not a background assumption but an output candidate of the discrete theory, albeit one whose strongest current support is mixed: arithmetic compilation plus native structural argument plus one still-open analytic step.
