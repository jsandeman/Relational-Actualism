...
Nature presents masses, radii, lifetimes, interaction ranges, and matter-sector asymmetries. The aim of Paper~II is to confront that domain directly from RA's own primitives: DAG growth, the BDG acceptance kernel, the local ledger condition, and finite motif renewal. The paper is not written as if the Standard Model did not exist; it is written on the stronger claim that the Standard Model's successes define an empirical domain RA must address, but not the mechanism RA must borrow.
...
The active compiled support surface used positively in this paper is deliberately narrow:
\begin{itemize}[nosep]
    ...
    \end{itemize}
\end{itemize}
Those modules are sufficient to make the structural core of the present paper real rather than aspirational.
