...
\begin{abstract}
The five integers $(1, -1, 9, -16, 8)$ that uniquely define the
Benincasa--Dowker--Glaser discrete curvature action in four spacetime
dimensions determine the Standard Model. We show that the BDG local
topology of a 4D causal graph admits exactly five stable pattern types,
exhaustively verified over 124 extension cases in Lean~4, corresponding
to the five particle classes of the Standard Model. Three BDG depth layers
($N_2, N_3, N_4$) give exactly three generations; no fourth exists because
no $N_5$ exists in 4D. The alternating signs of the BDG coefficients
$(+,-,+,-,+)$, arising from inclusion-exclusion counting, force the gauge
group hierarchy $SU(3) \times SU(2) \times U(1)$.

From these integers alone we derive both Standard Model coupling constants:
$\alpha_s(m_Z) = 1/\!\sqrt{72} = 0.11785$ (PDG agreement $0.13\%$) and
$\alpha_{\mathrm{EM}}^{-1} = 137.036$ via a discrete Dyson equation
(agreement $0.00001\%$), with zero free parameters. A self-consistent
cascade through five BDG depth layers gives
$m_p = m_P\,\alpha_{\mathrm{EM}}^5/2^{28} = 941~\mathrm{MeV}$
($0.3\%$ error), the proton charge radius
$r_p = L_q\,\hbar/(m_p c) = 0.84~\mathrm{fm}$ ($0.03\%$ of CODATA),
and the pion mass $m_\pi \approx (2/3)^5\,m_p = 124~\mathrm{MeV}$
($11\%$ error) --- all from the Planck mass alone, without continuum
renormalization. The cascade exponent $2N_c - 1$ equals the confinement
cycle length $d + 1$ only in $d = 4$, providing a new uniqueness argument
for four spacetime dimensions.

Structural predictions: no proton decay (exact baryon conservation from
LLC on topological winding); maximal parity violation as a theorem of DAG
acyclicity; $\theta_{\mathrm{QCD}} = 0$ exactly (no axion needed); and
$\sum m_\nu \approx 59~\mathrm{meV}$ (testable with CMB-S4).
All results are verified across 176 Lean~4 theorems in eight proof files.
\end{abstract}
...
\paragraph{GR from BDG uniqueness.}
...
algorithm), the Einstein field equations are the unique consistent macroscopic
description. This is established by two independent routes: Lovelock's
theorem~\citep{Lovelock1971} in the continuum, and the RA-native BDG
uniqueness chain (L01+O01+L11 $\to$ Benincasa-Dowker $\to$
Einstein-Hilbert) entirely within the discrete framework
(RACL~\citep{Sandeman2026RACL}). General relativity is
...
\begin{enumerate}[label=(\roman*), noitemsep]
  ...
  \item \textbf{The discrete Dyson equation (IC30, closed).}
  The integer~137 is the bare BDG depth-ratio fixed point
  (Lean-verified: $144 - 7 = 137$, where $144 = 12^2$ is the
  diamond vertex count and $7 = L_g + L_q$ from the confinement
  lengths). The dressed coupling receives a fractional correction
  from angular screening by virtual processes. The BDG screening
  amplitude is $P_{\mathrm{acc}} \times c_2 = 0.548 \times 9 = 4.932$,
  giving:
  \begin{equation}
    \alpha_{\mathrm{EM}}^{-1}
    = \frac{137 + \sqrt{137^2 + 4\,P_{\mathrm{acc}}\cdot c_2}}{2}
    = \frac{137 + \sqrt{18769 + 19.73}}{2}
    = 137.036019 \quad[\text{CV}].
    \label{eq:alpha_ic30}
  \end{equation}
  The PDG~2024 value is $\alpha_{\mathrm{EM}}^{-1} = 137.035999$,
  giving agreement to 0.00001\%. The near-identity
  $P_{\mathrm{acc}} \times c_2 \approx \pi^2/2$ is a
  \emph{consequence} of the BDG structure, not an input. No
  transcendental number ($\pi$, $e$, etc.) appears as a parameter
  at any stage; the entire derivation uses only integer arithmetic
  and the Poisson-CSG model at $\mu=1$.

  \item \textbf{Geometric origin (Wyler connection, open).}
  The Wyler formula
  \begin{equation}
...
The qualitative picture of asymptotic freedom and confinement above can be
made quantitative via a BDG-native renormalization group.

At density $\mu$, the effective BDG score of a Type-3 (quark) vertex
interpolates between the confinement limit (dominated by the vacuum
score $c_0$) and the asymptotic-freedom limit (dominated by the
free-vertex score $c_4$):
\begin{equation}
  S_{\mathrm{eff}}(\mu) \;=\;
  \bigl(1 - e^{-\mu}\bigr)\,c_4 + e^{-\mu}\,c_0\,.
  \label{eq:Seff_rg}
\end{equation}
The strong coupling at density $\mu$ is then:
\begin{equation}
  \alpha_s(\mu) \;=\; \frac{1}{\sqrt{c_2\,S_{\mathrm{eff}}(\mu)}}\,.
  \label{eq:alphas_mu}
\end{equation}
This model has two exact fixed points determined entirely by the BDG
integers $(c_0, c_2, c_4) = (1, 9, 8)$:
\begin{align}
  \alpha_s^{\mathrm{IR}} &= \lim_{\mu\to 0}\alpha_s(\mu)
    = \frac{1}{\sqrt{c_2\,c_0}} = \frac{1}{\sqrt{9}} = \frac{1}{3}
    \qquad(\text{confinement limit})\,,
  \label{eq:alpha_IR}\\[4pt]
  \alpha_s^{\mathrm{UV}} &= \lim_{\mu\to\infty}\alpha_s(\mu)
    = \frac{1}{\sqrt{c_2\,c_4}} = \frac{1}{\sqrt{72}} = 0.11785
    \qquad(\text{asymptotic-freedom limit})\,.
  \label{eq:alpha_UV}
\end{align}

\paragraph{Physical interpretation.}
The IR fixed point $\alpha_s^{\mathrm{IR}} = 1/3$ corresponds to the dense,
strongly-coupled confinement regime ($\mu\to 0$): colour charges clump and
the coupling is large.
The UV fixed point $\alpha_s^{\mathrm{UV}} = 1/\sqrt{72}$ corresponds to the
sparse, weakly-coupled asymptotic-freedom regime ($\mu\to\infty$): quarks
behave as nearly free particles and the coupling is small.
Both fixed points are exact consequences of the BDG integers with no external
parameters. The physically observed running of $\alpha_s$ from the hadronic
scale ($\alpha_s \sim 0.3$) to the electroweak scale ($\alpha_s(m_Z) \approx
0.118$) is bracketed by these two fixed points.

\paragraph{Comparison with the PDG value.}
The UV fixed point $\alpha_s^{\mathrm{UV}} = 1/\sqrt{72} = 0.11785$ agrees
with the PDG world average $\alpha_s(m_Z) = 0.11800 \pm 0.0009$~\citep{PDG2024}
to $0.13\%$, well within the experimental uncertainty.
The agreement is consistent with $m_Z$ lying in the slowly-running,
weakly-coupled regime close to the UV fixed point of the BDG RG.
A first-principles derivation of the mapping from BDG density $\mu$ to
physical energy scale remains an open target (see Hard Wall~D3,
...
GR inevitable (BDG uniqueness + Lovelock) & Proved & RAGC, RACL \\
...
Exact baryon conservation & Lean-verified (\texttt{RA\_BaryonChirality.lean}) & Prop.~\ref{prop:baryon} \\
Maximal parity violation & Lean-verified (\texttt{RA\_BaryonChirality.lean}) & Prop.~\ref{prop:chirality} \\
...
$\alpha_s^{\mathrm{UV}} = 1/\sqrt{72}$, $\alpha^{-1}_{\mathrm{EM}} = 137$ & BDG RG UV fixed point; BDG depth-ratio (Lean) & \S\ref{sec:bdg_rg}; Deriv.~3 \\
String tension $\sigma$ String tension $\sigma$ & Needs D2 simulation & Deriv.~3 Bypassed by $\mu^5$ cascade String tension $\sigma$ & Needs D2 simulation & Deriv.~3 Deriv.~2 \\
All fermion masses via $m_0$ chain & Cascade: $m_p = 941$~MeV ($0.3\%$) & Eq.~\eqref{eq:proton_mass_numerical} \\
...
The companion paper RATM~\citep{Sandeman2026RATM} provides a complete
Lean-verified proof. The D1\_closure\_complete theorem exhaustively
verifies all 124 single-step extensions of all particle types,
establishing that the five-element set is closed. Colour confinement
is proved with finite lengths $L=3$ (gluon) and $L=4$ (quark).
The original three sub-targets are all resolved:
\begin{enumerate}[noitemsep]
\item \emph{Completeness}: proved by D1\_closure\_complete (Lean, 0 sorry).
\item \emph{Multiplicity / charge assignment}: Gell-Mann--Nishijima
  verified for all six first-generation fermions (Lean).
\item \emph{Topological charge mechanism}: the asymmetric Y-join
  $(2,1,0,0)$ carries $N_1=2$ degeneracy encoding three colour charges;
  confinement length $L_q=4$ proved (Lean). Colour charge is the
  BDG-native analogue of non-abelian gauge structure: it
...
\paragraph{Derivation 2: The fermion mass chain --- the $\mu^5$ cascade.}
\label{par:deriv2}

The proton mass derives from a multiplicative cascade through the BDG depth layers.
The proton is sustained by gluon-exchange vertices $N = (1,2,0,0)$ (score $S = 18$);
at the proton's self-consistent internal actualization density $\mu_p$, the
Poisson probability of generating such a vertex is:
\begin{equation}
  P(\text{gluon} \mid \mu_p)
  = \frac{\mu_p^5}{8}\,\exp\Bigl(-\sum_{k=1}^{4}\frac{\mu_p^k}{k!}\Bigr)
  \;\approx\; \frac{\mu_p^5}{c_4}
  \qquad (\mu_p \ll 1).
  \label{eq:gluon_vertex_prob}
\end{equation}
The $\mu_p^5$ scaling is the \emph{cascade}: one power of $\mu$ for $N_1 = 1$
(one direct causal ancestor) and four powers for $N_2 = 2$
($\mathrm{Pois}(\mu^2/2)$ evaluated at $k=2$ gives $\mu^4/8$).
The proton mass equals the Planck mass suppressed by this gluon vertex rate:
\begin{equation}
  \boxed{\;\frac{m_p}{m_P}
  \;=\; \frac{\mu_p^5}{c_4}\,}
  \label{eq:proton_mass_cascade}
\end{equation}
with $c_4 = 8$ the BDG fourth-layer coefficient. This is the RA-native mechanism
for dimensional transmutation: five powers of a small density parameter produce the
twenty orders of magnitude separating the proton mass from the Planck mass,
with no renormalization group running and no continuum QCD input.

\textbf{Self-consistency determines $\mu_p$.}
The proton is a self-sustaining pattern: its own actualization events create the
internal density that sustains its topology. The self-consistency condition
$\mu_p = \rho_p\,V_{\mathrm{coh}}\,p_{\mathrm{th}}$ relates $\mu_p$ to the
proton's mass and radius. The four-dimensional actualization density inside
the proton is $\rho_p = N_{\mathrm{eff}}\,m_p/\bigl(\tfrac{4\pi}{3}r_p^3\bigr)$,
the Alexandrov coherence volume is $V_{\mathrm{coh}} = (\pi/4!)\,l_C^4$, and
$p_{\mathrm{th}} = \alpha_{\mathrm{EM}}$. Combining these, the $\pi$ from the
Alexandrov four-volume cancels exactly against the $\pi$ from the spherical
three-volume, giving the purely integer geometric factor:
\begin{equation}
  \mu_p
  \;=\; \frac{N_{\mathrm{eff}}\,\alpha_{\mathrm{EM}}}{32\;L_q^3}\,.
  \label{eq:mu_p_self_consistent}
\end{equation}
Setting $r_p = L_q\,l_C$ (the confinement radius equals $L_q$ Compton wavelengths)
eliminates the mass and radius, leaving $\mu_p$ determined by $N_{\mathrm{eff}}$,
$\alpha_{\mathrm{EM}}$, and $L_q$ alone.

\textbf{$N_{\mathrm{eff}} = 64$: a conjecture.}
The exact proton mass is recovered at $N_{\mathrm{eff}} = 64.0 = L_q^3 = 4^3$.
With this value the $L_q$ dependence cancels, giving the remarkably clean
\begin{equation}
  \mu_p = \frac{\alpha_{\mathrm{EM}}}{32}\,,
  \label{eq:mu_alpha32}
\end{equation}
and substituting into equation~\eqref{eq:proton_mass_cascade}:
\begin{equation}
  \boxed{\;m_p
  \;=\; \frac{m_P\,\alpha_{\mathrm{EM}}^5}{2^{28}}
  \;=\; 941~\mathrm{MeV}\;}
  \qquad (\text{observed:}~938.3~\mathrm{MeV};\;\text{error}~0.3\%).
  \label{eq:proton_mass_numerical}
\end{equation}
The exponent $28 = 5 \times 5 + 3$: five powers of $32 = 2^5$ from the
self-consistent density, plus $2^3 = c_4$ from the gluon vertex normalization.
The only inputs are $m_P$ (the single dimensionful scale in RA) and
$\alpha_{\mathrm{EM}} = 1/137.036$ (derived in Section~\ref{sec:ic30}).

The interpretation of $N_{\mathrm{eff}} = L_q^3 = 64$ is that the proton's
confinement region has $L_q^3$ independent actualization sites in
BDG depth-space --- the three-dimensional volume of the confinement lattice.
This is conjectured, not derived from BDG dynamics.
Deriving $N_{\mathrm{eff}}$ from first principles remains the primary open
target of the cascade programme.

\textbf{The proton radius.}
The confinement radius $r_p = L_q \times \hbar/(m_p c) = 4 \times 0.210~\mathrm{fm}
= 0.84~\mathrm{fm}$ agrees with the CODATA charge radius
$0.8414 \pm 0.0019~\mathrm{fm}$ to $0.03\%$.

\textbf{The neutron mass.}
The neutron has identical BDG topology to the proton (both bound by gluon
exchange at the same density $\mu_p$). The cascade formula predicts
$m_n = m_p$ to leading order; the observed splitting
$\Delta m = 1.293~\mathrm{MeV}$ is an electromagnetic correction from the
$u$-$d$ charge difference, consistent with standard QCD.

\textbf{The pion mass.}
If mesons (two quarks) have $N_{\mathrm{eff}} = \tfrac{2}{3} \times 64 = 42.7$
(scaling by quark count), then $\mu_\pi = \tfrac{2}{3}\,\mu_p$ and
\begin{equation}
  m_\pi = \Bigl(\frac{2}{3}\Bigr)^{\!5} m_p
  = \frac{32}{243}\times 941
  \approx 124~\mathrm{MeV}
  \qquad (\text{observed:}~140~\mathrm{MeV};\;\text{error}~11\%).
  \label{eq:pion_mass}
\end{equation}
The $11\%$ discrepancy likely reflects chiral symmetry breaking corrections
that the simple quark-count scaling does not capture. Non-strange vector
mesons ($\rho$, $\omega$) have $N_{\mathrm{eff}} \approx 62$, nearly equal
to the nucleon, consistent with their masses lying close to $m_p$.

\textbf{$d = 4$ uniqueness of the cascade exponent.}
The cascade exponent is $1 + 2N_2 = 2N_c - 1$ (from the gluon vertex requiring
$N_1 = 1$ and $N_2 = N_c - 1 = 2$). The confinement cycle length is $d + 1$.
The identity $2N_c - 1 = d + 1$ holds \emph{only} in $d = 4$ dimensions
(since $N_c = d - 1$), providing a new argument for the uniqueness of four
spacetime dimensions, independent of the D4U02 selectivity argument.

\textbf{Downstream.}
Closing the Koide chain $m_0 = m_p/3$ with the derived $m_p$
determines all charged lepton masses from BDG integers plus
$\alpha_{\mathrm{EM}}$ and the Koide angle $\theta$ (Section~\ref{sec:koide}).
The fermion mass hierarchy is the $\mu^5$ cascade; no fine-tuning,
no naturalness problem, no hierarchy problem.
...
coefficient requires the effective RA area element $l_{\mathrm{RA}}$.
...
\paragraph{Derivation 3: Coupling constants from BDG integers (established for $\alpha_s^{\mathrm{UV}}$ and $\alpha_{\mathrm{EM}}$; scale identification open).}

\textbf{Established result (BDG RG, new).}
The strong coupling constant UV fixed point is now a proved result of the
BDG renormalization group (Section~\ref{sec:bdg_rg}):
$\alpha_s^{\mathrm{UV}} = 1/\sqrt{c_2 c_4} = 1/\sqrt{72} = 0.11785$,
agreeing with the PDG world average $\alpha_s(m_Z) = 0.11800$ to $0.13\%$.
The IR fixed point $\alpha_s^{\mathrm{IR}} = 1/3$ reproduces the
strongly-coupled hadronic regime. Both follow from the five BDG integers alone.

\textbf{Established result ($\alpha_{\mathrm{EM}}$, optical theorem).}
The per-link actualization probability $p_{\mathrm{th}}$ has been identified
with the fine structure constant via the optical theorem
(Conjecture~\ref{conj:wyler}, condition~(i)):
\begin{equation}
  p_{\mathrm{th}} = \alpha_{\mathrm{EM}} \approx \frac{1}{137}.
\end{equation}
This removes $p_{\mathrm{th}}$ as an independent free parameter of RASM.
Previously calibrated from quantum error correction thresholds ($p_{\mathrm{th}} \approx 0.01$),
it is now identified with a fundamental constant via an RA-native argument:
the fraction of EM interaction candidates satisfying the actualization
criterion $\Delta S(\rho\|\sigma_0) > 0$ equals $\alpha_{\mathrm{EM}}$ by
the optical theorem.

Combined with the score-$1$ floor theorem (RATM, Lean-proved),
this gives the self-consistency condition $\mu_{\mathrm{stable}} = 1/\alpha_{\mathrm{EM}} \approx 137$:
the BDG-filtered Poisson-CSG at stationarity (with score-$1$ particles
dominating) has a mean parent count equal to $1/\alpha_{\mathrm{EM}}$.

\textbf{Established factors in the Wyler formula.}
$N_{\mathrm{forces}} = 3$ follows from the D1 stable pattern catalogue
(Section~\ref{sec:open}, Derivation~1): exactly three non-gravitational
$N$-vector degrees of freedom produce stable patterns in 4D.
$d = 4$ is the unique dimension giving a minimal closed stable catalogue
(established in Section~\ref{sec:topology_of_matter}).
The Lean-proved coherent state amplitude $|\hat\phi(n{=}1)|^2 = 3/2$
(Lemma~\ref{lem:coherent}) contributes the numerator in the coupling ratio.
The $\Gamma(5/4)$ factor appears in both the Wyler formula and the BDG
locality lemma (RACL Lemma~3.1), both arising from the 4D causal diamond
integral — a structural connection independent of any external input.

\textbf{Open.}
The remaining step is computing the $SU(3)_{\mathrm{gen}}$ coherent-state
phase-space volume in the BDG normalisation to derive (iii) of
Conjecture~\ref{conj:wyler}. When closed, all coupling constants become
native RA predictions derived from the BDG integers $(1,-1,9,-16,8)$ alone,
with $p_{\mathrm{th}}$ no longer an external input.
\textbf{Collaborator target:} differential geometry (symmetric spaces);
the Wyler programme~\citep{Wyler1969}; Lean~4 functional calculus.
...
These are structural results, not phenomenological fits. Of the four
original derivation targets, Derivation~1 (pattern enumeration) is
Lean-verified and Derivation~3 (coupling constants) is substantially
resolved; Derivation~2 (fermion mass chain) is partially closed: $m_p = m_P\,\alpha_{\mathrm{EM}}^5/2^{28} = 941$~MeV ($0.3\%$ error, $N_{\mathrm{eff}} = 64$ conjectured); the remaining
frontier. The question driving the next generation of this work is exact:
...
\newblock Review of Particle Physics.
\newblock \textit{Phys.\ Rev.\ D} \textbf{110}, 030001 (2024).
