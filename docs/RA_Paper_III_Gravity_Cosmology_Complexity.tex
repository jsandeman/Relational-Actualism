...
\begin{abstract}
The third paper of the Relational Actualism suite develops gravity, cosmology, causal severance (the RA treatment of horizons and singularities), black hole entropy, and cosmological nucleation. General relativity is derived via the RACL chain: LLC (Lean-verified) + amplitude locality (Lean-verified) + $d=4$ (BDG closure) + Benincasa--Dowker theorem, yielding $G_{\mu\nu}=8\pi G\,P_{\mathrm{act}}[T_{\mu\nu}]$ with $\Lambda=0$ structurally. The two apparent hard walls---the continuum Bianchi identity and Lorentz invariance---dissolve: Bianchi IS the LLC in continuum language, and Lorentz IS causal invariance (both Lean-verified) read through the sprinkling limit. Dark matter is reinterpreted as actualization-density topology: dimensional reduction at low $\mu$ via the Durhuus--Jonsson--Wheater theorem produces flat rotation curves from baryonic matter alone; the Bullet Cluster is explained by causal-depth sourcing (not particulate dark matter); the baryon-to-dark matter ratio $f_0=5.40$ is derived from BDG path-weight enumeration to 0.3\%. The cosmological constant problem, the Hubble tension, and WIMP dark matter are all addressed without free parameters. In a $\Lambda=0$ universe, cosmic voids expand as Milne while matter regions expand as Einstein--de~Sitter; the Milne luminosity distance fit with $\Lambda$CDM yields apparent $\Omega_\Lambda=0.68$ with zero free parameters (1.4\% match); the DESI evolving-$w$ signal ($w_0\approx -0.70$, $w_a\approx -0.90$) is structurally predicted, with distinctive environmental predictions (BAO void/filament splitting, $w$--void correlation). Heat death is prohibited: the universe fragments into causally disconnected patches via starvation and density severance before thermal equilibrium can be reached. Causal severance replaces singularities; the RA-native entropy observable is the count of severed links, $\SRa=|L_\Sigma|$, with discrete boundary law $\SRa\propto N_\partial$ (Bekenstein--Hawking as continuum translation). Kerr nucleation explains five CMB anomalies (low quadrupole, axis of evil, hemispheric asymmetry, normal octupole, dilution at high $\ell$) from three parameters, with testable predictions for CMB-S4 and DESI. Paper~IV develops the emergence of biological complexity from the same percolation threshold.
\end{abstract}
...
LV = Lean-verified. DR = derived (all steps explicit). No step is ``argued'' or conjectural. The derivation is as rigorous as the Benincasa--Dowker theorem, which is the one external input from causal set theory.

\subsection{The Bianchi identity IS the LLC}
...
The answer is that it does not ``arise''; it IS the LLC in continuum language.

\begin{proposition}[Bianchi--LLC dissolution]
\label{prop:bianchi_llc}
The discrete LLC and the continuum Bianchi identity are not two different conservation laws related by a limit procedure. They are the same conservation law expressed in different formalisms. The discrete LLC states: at every vertex, incoming flow equals outgoing flow. The Benincasa--Dowker continuum limit translates this into: $\nabla_\mu T^{\mu\nu} = 0$, which via the Einstein equation (eq.~\ref{eq:einstein}) implies $\nabla_\mu G^{\mu\nu} = 0$. The Bianchi identity is the LLC \emph{read through the continuum lens}, not an additional fact requiring proof.
\end{proposition}

This parallels a familiar situation in physics: Maxwell's equations can be written in four-component form ($F_{\mu\nu}$) or in three-component form ($\vec{E}, \vec{B}$). The three-component equation $\nabla \cdot \vec{B} = 0$ is not an additional fact beyond the four-component equation $\partial_\mu F^{\mu\nu}_{\mathrm{dual}} = 0$; it is the same equation in different notation. Similarly, the Bianchi identity is not an additional fact beyond the LLC; it is the LLC in continuum notation.

This dissolves what appeared to be a primary hard wall of the framework. The earlier framing---``we must prove $\nabla_\mu G^{\mu\nu} = 0$ holds in the continuum limit of the LLC''---imports a question (does the discrete identity survive the limit?) that conflates two levels of description. Once this conflation is identified, the question dissolves: yes, the discrete identity survives the limit, because it IS the continuum identity when read in continuum language.

\subsection{Lorentz invariance IS causal invariance}
...
Again, the apparent hard wall dissolves under the correct reading:

\begin{proposition}[Lorentz--causal dissolution]
\label{prop:lorentz_causal}
The quantum measure $\mu_\sigma(S)$ is foliation-independent: for any two linear extensions $\sigma, \sigma'$ of the causal order with respect to observables $S$, $\mu_\sigma(S) = \mu_{\sigma'}(S)$. This is Lean-verified (\texttt{causal\_invariance} in \texttt{RA\_AQFT\_Proofs\_v10.lean}, with the \texttt{amplitude\_locality} axiom closed in \texttt{RA\_AmpLocality.lean}). The continuum image of this statement, obtained via the Benincasa--Dowker sprinkling limit, is: physical observables are independent of the Lorentz frame. These are not two different invariances; they are the same invariance in different formalisms.
\end{proposition}

The discrete causal invariance says: the order in which spacelike-separated events are declared to have occurred does not affect the quantum measure of any set of observables. The continuum Lorentz invariance says: physical observables are independent of the reference frame. A reference frame is a choice of simultaneity surface, which in the discrete picture is a choice of linear extension of the causal order. The two statements are translations of one another.

This dissolves the second primary hard wall. RA's Lorentz invariance is not a conjecture to be verified against experiment; it is a Lean-verified theorem of discrete causal invariance, read through the continuum lens.
...
The framework does not require an independent derivation of Lorentz invariance in curved spacetime, because local Lorentz invariance is a direct consequence of causal invariance (\S\ref{sec:lorentz_is_causal}), which holds at every vertex of the DAG regardless of its global embedding.
...
Any particle that does not participate in on-shell electromagnetic or strong interactions has zero RA assembly depth---it never crosses the actualization threshold through normal matter interactions. Therefore:

\begin{proposition}[WIMP prohibition]
WIMPs, axions, and sterile neutrinos contribute zero weight to $P_{\mathrm{act}}[T_{\mu\nu}]$ and are categorically prohibited as gravitationally coupled dark matter candidates.
\end{proposition}

Current data from LZ and XENONnT are consistent with this prediction. RA predicts that no dark matter signal will be found below the neutrino floor in any direct detection experiment, for any candidate particle that does not participate in on-shell EM or strong interactions. This prediction is categorical, not probabilistic: it excludes not only WIMPs but any proposed dark matter candidate whose interactions with ordinary matter are below the on-shell threshold.
...
The detailed mechanism is captured by a boundary correction term in the metric-sourcing equation. In the weak-field quasi-static limit, the gravitational potential satisfies:
\begin{equation}
\label{eq:modified_poisson}
\nabla^2\Phi = 4\pi G\,\rho_m + \nabla^2(\ln\lambda)\,c^2,
\end{equation}
where $\lambda(x)$ is the local actualization density (a smoothed version of $\mu$). The first term is the standard Newtonian sourcing; the second term is the RA correction from the causal graph geometry.

In dense regions, $\lambda \approx \lambda_0$ (approximately constant), so $\nabla^2(\ln\lambda) \approx 0$ and the standard Poisson equation is recovered. In sparse galactic halos, $\lambda(r)$ falls off with radius, and $\nabla^2(\ln\lambda) > 0$: the correction term sources an additional effective gravitational field that mimics the presence of dark matter.

\begin{proposition}[Rotation curve mechanism]
\label{prop:rotation_curve}
A galaxy with baryonic matter density $\rho_m(r)$ concentrated in a disk of radius $R_d$, surrounded by a halo where $\lambda(r) \propto r^{-2}$ at large radii, produces a flat rotation curve with asymptotic velocity $v_\infty$ determined by $\lambda_0$ and $R_d$. No dark matter particles are required.
\end{proposition}

The transition radius between the inner Newtonian regime and the outer flat regime is set by the point where $\lambda(r)$ begins to fall sharply---typically at the edge of the visible disk. This reproduces the MOND-like phenomenology (Milgrom's $a_0$) as an emergent scale in the RA framework, rather than a fundamental acceleration constant.
...
The baryon-to-dark matter ratio is derived in Paper~II~\S\ref{sec:masses} as:
\begin{equation}
...
\begin{equation}
\frac{\bar{\lambda}_m}{\bar{\lambda}_{\mathrm{exp}}} = \frac{\Omega_b(1+f_0)}{1-\Omega_b(1+f_0)} = 0.463 \quad\text{(parameter-free)}.
\end{equation}
...
\textbf{T4 (Hubble tension as density gradient):} Already derived above: $H_{\mathrm{local}}=73.6$\,km/s/Mpc (parameter-free).
...
In RA, the Big Bang is a causal severance event---the nucleation of a new graph branch from a parent black hole. The parent's physics determines the daughter's initial conditions.
...
If the parent is a Kerr (rotating) black hole with dimensionless spin parameter $a^* \approx 0.9$, the anisotropic severance geometry predicts five CMB anomalies from a single mechanism. This subsection develops the quantitative model.
...
Three parameters, five anomalies, three testable predictions, zero remaining freedom. $\Lambda$CDM has no mechanism for any of these anomalies; they remain unexplained ``cosmic coincidences'' in standard cosmology.
...
\midrule
\multicolumn{3}{l}{\textit{Foundations (Lean-verified)}} \\
LLC at every vertex & Lean 4 & LV \\
Graph Cut Theorem & Lean 4 & LV \\
Markov Blanket Shielding & Lean 4 & LV \\
Causal invariance & Lean 4 + \texttt{RA\_AmpLocality.lean} & LV \\
Amplitude locality & Lean 4 & LV \\
Frame independence & Lean 4 & LV \\
Rindler stationarity / Unruh resolution & Lean 4 & LV \\
\midrule
\multicolumn{3}{l}{\textit{GR derivation}} \\
$G_{\mu\nu}=8\pi G P_{\mathrm{act}}[T_{\mu\nu}]$ & BDG uniqueness + $P_{\mathrm{act}}$ & DR \\
$\Lambda=0$ exactly & $P_{\mathrm{act}}$ structure & DR \\
Bianchi $\equiv$ LLC (dissolution) & Continuum reading of LLC & DR \\
Lorentz $\equiv$ causal invariance (dissolution) & Continuum reading of LV theorem & DR \\
Field equation uniqueness (no Lovelock) & BD theorem + $P_{\mathrm{act}}$ + LLC + $d{=}4$ & DR \\
\midrule
\multicolumn{3}{l}{\textit{Dark matter as topology}} \\
WIMP prohibition (categorical) & $P_{\mathrm{act}}$ structure & DR \\
Dimensional reduction at low $\mu$ & Durhuus-Jonsson-Wheater & DR \\
Flat rotation curves from $\nabla^2(\ln\lambda)$ & Modified Poisson & AR \\
Bullet Cluster (Weyl vs Ricci) & Causal depth source & AR \\
WEP preserved at $10^{-4}$ & Equilibration estimate & DR \\
$f_0=17.32\times\alpha_s(2m_p)=5.40$ & BDG path weights + SM RG & CV (0.3\%) \\
$\Omega_m=\Omega_b(1+f_0)\approx 0.31$ & From $f_0$ & CV (Planck: 0.315) \\
\midrule
\multicolumn{3}{l}{\textit{Severance and entropy}} \\
$\Dkl(K\|\mathrm{Pois})=\Delta S^*$ & Algebraic & Proved \\
$\TV(K,\mathrm{Pois})=1-\Pacc$ & Algebraic & Proved \\
$\Pacc\to 1$ as $\mu\to\infty$ & Chebyshev & Proved \\
Antichain drift $>0$ for $\mu<1.25$ & Algebraic + MC & Proved \\
$\SRa=|L_\Sigma|$ finite and invariant & Axiom 7 & Proved \\
$\SRa\propto N_\partial$ (discrete boundary law) & $\langle d_{\mathrm{out}}\rangle$ local & DR \\
$N_\partial\leftrightarrow A/(4l_P^2)$ & BDG-to-metric dictionary & DR \\
\midrule
\multicolumn{3}{l}{\textit{Cosmological predictions}} \\
$H_{\mathrm{local}}=73.6$\,km/s/Mpc & Parameter-free & CV \\
$\Omega_\Lambda^{\mathrm{app}}=0.68$ from Milne shape & Zero parameters, 1.4\% match & CV \\
$w_0=-0.71$, $w_a=-0.91$ (EdS$\to$Milne) & 1 parameter ($t_{\mathrm{trans}}$) & Phenomenological \\
...
Kerr nucleation: 5 CMB anomalies & 3 params, 5 predictions & Strong evidence \\
...
Cosmic web-CMB axis correlation (T2$'$) & Parent spin imprint & Prediction \\
$\alpha=0.68$ (release fraction) & $\eta_b$ fit & Phenomenological \\
\bottomrule
\end{tabular}
\end{center}

LV = Lean-verified. CV = computation-verified. DR = derived (all steps explicit). AR = argued (physically motivated, not all steps explicit). The RA cosmological programme is built on LV foundations with explicit DR extensions; the distinctive predictions are CV or prediction status.
...
From the BDG integers and the sequential growth rule, the present paper derives: general relativity with $\Lambda=0$ via the RACL chain (LLC + amplitude locality + $d=4$ + Benincasa--Dowker); the dissolution of the two apparent hard walls (Bianchi IS LLC, Lorentz IS causal invariance); the categorical prohibition of WIMPs; the dark matter phenomenology from causal-graph dimensional reduction at low $\mu$ (flat rotation curves, Bullet Cluster, $f_0 = 5.40$ from BDG path weights); the Hubble tension as a density-gradient effect; the apparent $\Omega_\Lambda=0.68$ from inhomogeneous expansion in a $\Lambda=0$ universe (zero free parameters, 1.4\% match); the DESI evolving-$w$ signal as the EdS-to-Milne transition ($w_0=-0.71$, $w_a=-0.91$ from one parameter); the complete expansion-to-severance lifecycle; the cosmic web as a dynamical attractor; the heat death prohibition via fragmentation; the RA-native entropy observable as severed-link count; the discrete boundary law $\SRa\propto N_\partial$; the Boltzmann Brain prohibition (structural + dynamical); and the Kerr nucleation model explaining five CMB anomalies from three parameters. The Bekenstein--Hawking formula $S=A/(4l_P^2)$ is the continuum translation of the discrete law, with the coefficient $1/4$ open pending the analytical determination of two Planck-scale geometric constants. The distinctive predictions---BAO void/filament splitting, $w$--void fraction correlation, cosmic web--CMB axis alignment, no Page curve---separate RA from $\Lambda$CDM, quintessence, and holographic alternatives simultaneously, and are testable with current and near-term data (DESI, Euclid, CMB-S4, LiteBIRD).

Paper~IV extends the same framework to biological complexity, showing that the $\mu=1$ percolation threshold governing QCD confinement and the actualization filter also governs the emergence of life.
