...
older component context's generic `orientationComponent` field.  The data are
...
structure DAGOrientationLinkSurfaceContext
    ...
    (C : DAGNativeCertificateComponentContext Ω) where
  orientationLinkEvidence : CausalSupportCut V → Type*
  orientationLinkRefinesComponent :
    ∀ Q, orientationLinkEvidence Q → C.orientationComponent Q
...
structure GraphOrientationLinkSurfaceContext
    ...
    (C : GraphNativeCertificateComponentContext Ω) where
