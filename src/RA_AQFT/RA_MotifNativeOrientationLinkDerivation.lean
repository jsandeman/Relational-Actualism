...
-- Note: the chainScore conflict between RA_D1_Core and RA_D1_NativeKernel
-- (RA-ISSUE-LEAN-CHAINSCORE-001) was resolved by wrapping RA_D1_NativeKernel
-- content in `namespace D1Native`, so all three native catalog modules can
-- now be imported simultaneously. Downstream instantiations that consume
-- D1Native content should `open D1Native` after their imports.
...
theorem DAGHasNativeOrientationLinkCatalog.to_orientation_component
    ...
    {Q : CausalSupportCut V}
    (h : DAGHasNativeOrientationLinkCatalog K Q) :
    Nonempty (C.orientationComponent Q) := by
