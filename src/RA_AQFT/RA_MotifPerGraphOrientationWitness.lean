...
def DAGHasPerGraphOrientationWitness
  ...
  ∃ i : P.memberIndex Q, Nonempty (P.perGraphOrientationEvidence g Q i)
...
theorem DAGHasPerGraphOrientationWitness.to_native_catalog
    ...
    (h : DAGHasPerGraphOrientationWitness P g Q) :
    DAGHasNativeOrientationLinkCatalog K Q := by
  rcases h with ⟨i, hi⟩
  rcases hi with ⟨e⟩
  exact ⟨P.refinesNativeCatalog g Q i e⟩
...
theorem GraphHasPerGraphOrientationWitness.to_native_catalog
    ...
    (h : GraphHasPerGraphOrientationWitness P g Q) :
