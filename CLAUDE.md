...
12. **Scan for >100 MB output files BEFORE staging.** Run `find <packet-root>/outputs -type f -size +50M -exec ls -lh {} \;` (or `du -ah <packet-root> | sort -rh | head`) before `git add`. Track B canonical-run packets routinely produce 100MB–2GB CSVs (e.g. `ra_trackB3b_sampler_sweep.csv` was 2.0GB). GitHub's hard limit is 100 MB per file. For each large file: (a) add the path to `.gitignore`; (b) flag the corresponding `artifacts.csv` row with `status=gitignored_reproducible` (preserve the SHA-256 for reproducibility); (c) record the run command + commit SHA so the file can be regenerated. If a large file is already in committed history when the limit is hit, the only fix is `git filter-repo --invert-paths --path …` followed by force-push (destructive — needs explicit user authorization). Catching it pre-commit avoids that pain.
13. **Don't push to `main`.** Direct push is policy-denied. Either open a PR (`gh pr create`) or hand the push back to the user; do not chase workarounds.
...
8. **Track B.1 packet bug — `Prop`-typed methodological field accessors are circular.** Track B.1 (`RA_MotifNativeGraphCutWitnessExtraction.lean`) shipped with structures whose Prop-valued fields (e.g. `graph_cut_derived : Prop`, `same_graph_cut_domain : Prop`) had accessor theorems trying to return the field as if it were already a proof: `(W : Witness) : W.graph_cut_derived := W.graph_cut_derived` — but `W.graph_cut_derived` is the *proposition*, not its proof. Fix: add paired proof fields (`graph_cut_derived_proof : graph_cut_derived`, etc.) and have the accessors return `W.graph_cut_derived_proof`. Generalises to any packet that introduces methodological-marker Prop fields.

## Recurring Python packet fixes (analysis-module bugs)

These three patterns form a "multi-source CSV merge" cluster. They appear together in cross-track analysis packets (e.g. anything that concats v1.7 keyed_trial_rows with B.x sampler outputs). Combined they can produce *plausible-looking but completely wrong* aggregate findings — including 6-decimal byte-identity between graph and shuffled control. Skepticism heuristic: identity to 6 decimals across 9+ independent cells means data-plumbing bug, not scientific result.

1. **Scalar-to-empty-DataFrame leak.** A normalizer that builds an output via `out = pd.DataFrame()` then assigns scalars (`out["config_id"] = "foo"`) to that empty DataFrame *before* the first length-N Series assignment leaves those columns as NaN forever — when a later `out["x"] = df.get(...)` extends the index, the scalar columns are not retroactively broadcast. Diagnostic: run summary reports `source_count=0` (or any nunique-zero) for a column the code "obviously" set. Fix: pass `index=range(len(df))` to the constructor: `out = pd.DataFrame(index=range(len(df)))`.
2. **Multi-source CSV first-match column collision.** When the input is `pd.concat([source_A, source_B], sort=False)` and each source ships the same conceptual field under a different column name (e.g. v1.7 has `v1_7_keying`, B.3b has `keying`), `_find_col(out, ["keying", "v1_7_keying", ...])` picks the first present column. Rows from the OTHER source then have NaN there and are silently excluded by any downstream filter. Diagnostic: per-source row counts in the merged frame are short by exactly one source's contribution; `df["source_file"].value_counts()` reveals it. Fix: coalesce candidate columns per-row using `.fillna(out[c])` for every other candidate.
3. **Candidate-list priority when keying-specific and keying-agnostic columns coexist.** Some sources (v1.7 keyed_trial_rows in particular) ship BOTH a keying-AGNOSTIC native simulator column (`orientation_overlap`, identical across all keying replicas of a trial) AND a keying-SPECIFIC column (`v1_7_orientation_overlap_all_pairs`, varies across replicas). If `_find_col` puts the native column FIRST, the analysis silently bins on a column that doesn't depend on keying — graph and control then partition identically by construction. Fix: keying-specific columns must come first in the candidate list; native columns are fallbacks for sources that don't provide a keying-specific equivalent. Diagnostic: probe the merged frame and check whether the column the script reports as `overlap_column_used` is the keying-specific one.
...
- Don't `git add <packet-root>/outputs/` blind. Run `find <packet-root>/outputs -type f -size +50M` first; gitignore + flag (`status=gitignored_reproducible`, preserve SHA-256) anything > ~50 MB. Track B taught us a 305 MB and a 2.0 GB CSV will cheerfully sit in `outputs/` and silently push-fail at the GitHub 100 MB hook, forcing a `git filter-repo` + force-push to recover.
- Don't believe a "byte-identical graph vs shuffled-control" finding without verifying that the binning column actually varies across keyings. `_find_col` picking a keying-agnostic column over a keying-specific one will manufacture exactly that pattern. Probe `df.attrs.get("overlap_column_used")` (or the equivalent) and a per-trial `merge` between graph and control rows before recording the finding. Skepticism heuristic: 6-decimal identity across 9+ independent cells = data-plumbing bug, not result.
